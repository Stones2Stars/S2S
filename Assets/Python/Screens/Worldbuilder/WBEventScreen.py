...
class WBEventScreen:
	...
	def placeEvents(self):
		...
		screen.setLabel("EventText", "Background", "<font=3b>" + sHeader + "</font>", 1<<2, screen.getXResolution() /10, self.iTable_Y - 30, -0.1, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		...
		for item in lEvents:
			...
			if item[1] == iSelectedEvent:
				sColor = CyTranslator().getText("[COLOR_POSITIVE_TEXT]", ())
				sHeader = item[0]
			screen.setTableText("WBEvent", 0, iRow, "<font=3>" + sColor + item[0] + "</font></color>", "", WidgetTypes.WIDGET_PYTHON, 1030, item[1], 1<<0 )
		if iSelectedEvent > -1:
