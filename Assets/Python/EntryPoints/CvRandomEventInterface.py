	...
	for loopCity in GC.getPlayer(data.ePlayer).cities():
		if loopCity.getPopulation() >= 5:
			if ENABLER.getBuildingAvailability(loopCity.getOwner(), loopCity.getID(), iBuilding) == EnablerState.ENABLER_LISTED:
				loopCity.setBuilding(iBuilding, True)
...
def getHelpWeddingFeud2(argsList):
	data = argsList[1]
	return TRNSLTR.getText("TXT_KEY_EVENT_WEDDING_FEUD_2_HELP", (GC.getDefineINT("TEMP_HAPPY"), 30, TEXT.getSymbolChar("RELIGION_", data.eReligion)))
...
def getHelpWeddingFeud3(argsList):
	data = argsList[1]
	return TRNSLTR.getText("TXT_KEY_EVENT_WEDDING_FEUD_3_HELP", (1, TEXT.getSymbolChar("RELIGION_", data.eReligion)))
...
def applyLooters3(argsList):
	...
	while aList and iCount < iMax:
		...
		CyCity.setBuilding(iBuilding, False)
...
def applyTsunami1(argsList):
	GC.getPlayer(argsList[1].ePlayer).getCity(argsList[1].iCityId).disband()
...
def canTriggerChampionUnit(argsList):
	iPlayer = argsList[1]
	iUnit = argsList[2]
	aRead = GC.getPlayer(iPlayer).getUnit(iUnit).getRead()
	...
	if aRead[UnitReadKind.UNIT_READ_HP] < aRead[UnitReadKind.UNIT_READ_MAX_HP] or aRead[UnitReadKind.UNIT_READ_LEVEL] < 5:
		return False
	if GC.getPlayer(iPlayer).getUnit(iUnit).hasPromotion(GC.getInfoTypeForString("PROMOTION_LEADERSHIP")):
		return False
...
def applyChampion(argsList):
	data = argsList[1]
	GC.getPlayer(data.ePlayer).getUnit(data.iUnitId).setPromotion(GC.getInfoTypeForString("PROMOTION_LEADERSHIP"), True)

def getHelpChampion(argsList):
	data = argsList[1]
	iLeadership = GC.getInfoTypeForString("PROMOTION_LEADERSHIP")
	return TRNSLTR.getText("TXT_KEY_EVENT_CHAMPION_HELP", (GC.getPlayer(data.ePlayer).getUnit(data.iUnitId).getName(), INFO.getTextKey("PROMOTION_", iLeadership)))
...
def getHelpAntelope2(argsList):
	...
	return TRNSLTR.getText("TXT_KEY_EVENT_IMPROVEMENT_GROWTH", ( INFO.getTextKey("IMPROVEMENT_", iCamp), ))
...
def getHelpIndependentFilms(argsList):
	return TRNSLTR.getText("TXT_KEY_EVENT_INDEPENDENTFILMS_HELP_1", (1, TEXT.getSymbolChar("BONUS_", GC.getInfoTypeForString("BONUS_HIT_MOVIES")), GC.getPlayer(argsList[1].ePlayer).getCity(argsList[1].iCityId).getNameKey()))
...
def canTriggerSecurityTax(argsList):
  ...
  if player.getNumCities() > GC.getPlayer(data.ePlayer).getBuildingCount(iWalls):
    return False
...
def canTriggerLiteracy(argsList):
  ...
  if player.getNumCities() > GC.getPlayer(data.ePlayer).getBuildingCount(iLibrary):
    return False
...
def canTriggerHorseWhisperingDone(argsList):
	return (
		...
		GC.getPlayer(argsList[0].ePlayer).getBuildingCount(GC.getInfoTypeForString("BUILDING_STABLE"))
	)
...
def canTriggerHarbormasterDone(argsList):
  ...
  iHarborsRequired = WORLD.getDefaultPlayers(GC.getMap().getWorldSize())
  if iHarborsRequired > GC.getPlayer(data.ePlayer).getBuildingCount(iHarbor):
    return False
...
def canTriggerClassicLiteratureDone(argsList):
	return (
		WORLD.getDefaultPlayers(GC.getMap().getWorldSize()) <=
		GC.getPlayer(argsList[0].ePlayer).getBuildingCount(GC.getInfoTypeForString("BUILDING_LIBRARY"))
	)
...
def canTriggerMasterBlacksmithDone(argsList):
	...
	if WORLD.getDefaultPlayers(GC.getMap().getWorldSize()) > GC.getPlayer(data.ePlayer).getBuildingCount(GC.getInfoTypeForString("BUILDING_FORGE")):
		return False
	...
	if isLocalHumanPlayer(data.ePlayer) :
		CyInterface().addMessage(
			...
			TRNSLTR.getText(
				...
				(
					GC.getPlayer(data.ePlayer).getCity(data.iCityId).getNameKey(), INFO.getTextKey("BONUS_", iBonus)
				)
			),
			"AS2D_DISCOVERBONUS", InterfaceMessageTypes.MESSAGE_TYPE_MINOR_EVENT, INFO.getButton("BONUS_", iBonus),
...
def canTriggerBestDefenseDone(argsList):
  ...
  if iBuildingsRequired > GC.getPlayer(data.ePlayer).getBuildingCount(iCastle):
    return False
...
def canTriggerSportsLeagueDone(argsList):
  ...
  iBuildingsRequired = WORLD.getDefaultPlayers(GC.getMap().getWorldSize())
  if iBuildingsRequired > GC.getPlayer(data.ePlayer).getBuildingCount(iColosseum):
    return False
...
def getHelpCrusadeDone2(argsList):
	data = argsList[1]
	return TRNSLTR.getText("TXT_KEY_EVENT_CRUSADE_DONE_HELP_2", (INFO.getTextKey("BUILDING_", data.eBuilding), GAME.getHolyCity(data.eReligion).getNameKey()))
...
def applyCrusadeDone2(argsList):
  ...
  holyCity = GAME.getHolyCity(data.eReligion)
  holyCity.setBuilding(data.eBuilding, True)
...
def getHelpSecretKnowledge2(argsList):
	data = argsList[1]
	return TRNSLTR.getText("TXT_KEY_EVENT_YIELD_CHANGE_BUILDING", (INFO.getTextKey("BUILDING_", data.eBuilding), u"+4[ICON_CULTURE]"))
...
def canTriggerExperiencedCaptain(argsList):
  ...
  player = GC.getPlayer(data.ePlayer)
  aUnit = GC.getPlayer(data.ePlayer).getUnit(data.iUnitId).getRead()
...
def getHelpGreed1(argsList):
	...
	iTurns = GAME.getSpeedPercent()

	return TRNSLTR.getText("TXT_KEY_EVENT_GREED_HELP_1", (CyPlayerOther.getCivilizationShortDescriptionKey(), INFO.getTextKey("BONUS_", iBonus), iTurns))
...
def expireGreed1(argsList):
	...
	if GAME.getGameTurn() >= data.iTurn + GAME.getSpeedPercent():
		return True
...
def getHelpGreedDone1(argsList):
	...
	if iUnitType != -1:
		szHelp = TRNSLTR.getText("TXT_KEY_EVENT_GREED_DONE_HELP_1", (iNumUnits, INFO.getTextKey("UNIT_", iUnitType)))
	return szHelp
...
def canApplyWarshipsDone2(argsList):
  ...
  iBuilding = GC.getInfoTypeForString("BUILDING_THE_GREAT_LIGHTHOUSE")
  if GC.getPlayer(data.ePlayer).getBuildingCount(iBuilding) == 0:
    return False
...
# szHelp = TRNSLTR.getText("TXT_KEY_EVENT_OVERWHELM_HELP_1", (iNumDestroyers, INFO.getTextKey("UNIT_", iDestroyer), iNumBattleships, INFO.getTextKey("UNIT_", iBattleship), iNumCarriers, INFO.getTextKey("UNIT_", iCarrier), iNumFighters, INFO.getTextKey("SPECIALUNIT_", iFighter), INFO.getTextKey("PROJECT_", iProject)))
  szHelp = TRNSLTR.getText("TXT_KEY_EVENT_OVERWHELM_HELP_1", (iNumDestroyers, INFO.getTextKey("UNIT_", iDestroyer), iNumBattleships, INFO.getTextKey("UNIT_", iBattleship), iNumCarriers, INFO.getTextKey("UNIT_", iCarrier), iNumFighters, INFO.getTextKey("SPECIALUNIT_", iFighter), INFO.getTextKey("BUILDING_", iBuilding)))
...
def getHelpCorporateExpansionDone1(argsList):
	data = argsList[1]
	return TRNSLTR.getText("TXT_KEY_EVENT_YIELD_CHANGE_BUILDING", (INFO.getTextKey("BUILDING_", data.eBuilding), u"+50[ICON_GOLD]"))
...
def getHelpHostileTakeover1(argsList):
  ...
  for iBonus in listResources:
    if not bFirst:
      ...
      bFirst = False
    szList += u"[COLOR_HIGHLIGHT_TEXT]" + INFO.getDescription("BONUS_", iBonus) + u"[COLOR_REVERT]"
...
def getHelpHostileTakeoverDone1(argsList):
	data = argsList[1]
	return TRNSLTR.getText("TXT_KEY_EVENT_YIELD_CHANGE_BUILDING", (INFO.getTextKey("BUILDING_", data.eBuilding), u"+100[ICON_GOLD]"))
...
def getHelpGreatBeast3(argsList):
	data = argsList[1]
	return TRNSLTR.getText("TXT_KEY_EVENT_GREAT_BEAST_3_HELP", (GC.getDefineINT("TEMP_HAPPY"), 40, TEXT.getSymbolChar("RELIGION_", data.eReligion)))
...
def doNuclearProtest1(argsList):
	...
	for loopUnit in GC.getPlayer(data.ePlayer).units():
		if loopUnit.getRead()[UnitReadKind.UNIT_READ_TYPE] == iICBM or loopUnit.getRead()[UnitReadKind.UNIT_READ_TYPE] == iTacNuke:
...
def canApplyBillionsandBillions2(argsList):
	data = argsList[1]
	return GC.getPlayer(data.ePlayer).getBuildingCount(GC.getInfoTypeForString("BUILDING_HOLLYWOOD")) != 0
...
def canTriggerRubiconUnit(argsList):
  ...
  aUnit = GC.getPlayer(ePlayer).getUnit(iUnit).getRead()
  ...
  if GC.getPlayer(ePlayer).getUnit(iUnit).hasPromotion(iMorale):
    return False
...
def getHelpRubicon2(argsList):
	...
	iMorale = GC.getInfoTypeForString("PROMOTION_MORALE")
	return TRNSLTR.getText("TXT_KEY_EVENT_CROSSING_THE_RUBICON_2_HELP", (unit.getNameKey(), INFO.getTextKey("PROMOTION_", iMorale)))
...
def applyRubicon2(argsList):
	...
	iMorale = GC.getInfoTypeForString("PROMOTION_MORALE")
	unit2.setPromotion(iMorale, True)
...
def canDoGeneralsPutsch1(argsList):
  ...
  EifCnt = GC.getPlayer(data.ePlayer).getBuildingCount(iEiffel)
  BrodCnt = GC.getPlayer(data.ePlayer).getBuildingCount(iBroadcast)
...
def canTriggerCureforCancer(argsList):
	...
	if pPlayer.isCivic(GC.getInfoTypeForString("CIVIC_DIVINE_CULT")):
		return False
	if GC.getPlayer(data.ePlayer).getBuildingCount(GC.getInfoTypeForString("BUILDING_NATIONAL_MEDICAL_DATABASE")) == 0:
		return False
...
def canTriggerSyntheticFuels(argsList):
	...
	or GC.getPlayer(data.ePlayer).getBuildingCount(GC.getInfoTypeForString("BUILDING_HQ_STANDARD_ETHANOL")) > 0
	):
		return False
...
def getHelpSyntheticFuels1(argsList):
	data = argsList[1]
	return TRNSLTR.getText("TXT_KEY_EVENT_SYNTHETIC_FUELS_HELP_1", ( 1, TEXT.getSymbolChar("BONUS_", GC.getInfoTypeForString("BONUS_OIL")), GC.getPlayer(data.ePlayer).getCity(data.iCityId).getNameKey()))

def getHelpSyntheticFuels2(argsList):
	data = argsList[1]
	return TRNSLTR.getText("TXT_KEY_EVENT_SYNTHETIC_FUELS_HELP_2", ( 1, TEXT.getSymbolChar("BONUS_", GC.getInfoTypeForString("BONUS_OIL")), GC.getPlayer(data.ePlayer).getCity(data.iCityId).getNameKey()))
...
def getHelpSyntheticFuels4(argsList):
	...
	return TRNSLTR.getText("TXT_KEY_EVENT_SYNTHETIC_FUELS_HELP_4", (1, TEXT.getSymbolChar("BONUS_", oBonus), pCity.getNameKey()))
...
def canTriggerAlternativeEnergy(argsList):
	...
	if GC.getPlayer(argsList[0].ePlayer).getBuildingCount(GC.getInfoTypeForString("BUILDING_THREE_GORGES_DAM")):
		return False
	if not GC.getPlayer(argsList[0].ePlayer).getBuildingCount(GC.getInfoTypeForString("BUILDING_COAL_PLANT")):
		return False
	return True
...
def canTriggerAlternativeEnergyDone(argsList):
	...
	if iBuildingsRequired > (GC.getPlayer(data.ePlayer).getBuildingCount(iHydro) + GC.getPlayer(data.ePlayer).getBuildingCount(iNuke)):
		return False
...
def canTriggerPiratesoftheNeutralZones(argsList):
	...
	for CyUnit in CyPlayer.units():
		if CyUnit.getRead()[UnitReadKind.UNIT_READ_DOMAIN] == DomainTypes.DOMAIN_SEA:
			iNavy += CyUnit.getBaseCombatStr()
...
def applyMalaccanPirates1(argsList):
  ...
  for loopUnit in spawnedUnits:
      ...
      GC.getPlayer(iOwner).getUnit(iUnitId).setPromotion(iNav1, True)
      GC.getPlayer(iOwner).getUnit(iUnitId).setPromotion(iCbt4, True)
      GC.getPlayer(iOwner).getUnit(iUnitId).setPromotion(iCoAs1, True)
      GC.getPlayer(iOwner).getUnit(iUnitId).setName(u"Malaccan Gunboat")
...
def applyHenryMorgan1(argsList):
  ...
  for i in xrange(iNumUnit1):
      ...
      CyUnit.setPromotion(iCbt4, True)
...
def applyStedeBonnet1(argsList):
  ...
  for i in xrange(iNumUnit2):
      ...
      CyUnit.setPromotion(iNav1, True)
...
def canTriggerV_Ger(argsList):
	...
	if GC.getPlayer(data.ePlayer).getBuildingCount(iObsrvty) < 1:
		return False
...
def TriggerHarryPotter2(argsList):
	...
	for CyCity in CyPlayer.cities():
		if ENABLER.getBuildingAvailability(CyCity.getOwner(), CyCity.getID(), iLibrary) == EnablerState.ENABLER_LISTED:
			CyCity.setBuilding(iLibrary, True)
...
def getHelpLessDictator2(argsList):
	...
	if CyPlayer.canDoCivics(iFederal):
		return TRNSLTR.getText("TXT_KEY_EVENT_CHANGE_CIVICS", (INFO.getTextKey("CIVIC_", iFederal),))

	iDemocracy = GC.getInfoTypeForString("CIVIC_DEMOCRACY")
	if CyPlayer.canDoCivics(iDemocracy):
		return TRNSLTR.getText("TXT_KEY_EVENT_CHANGE_CIVICS", (INFO.getTextKey("CIVIC_", iDemocracy),))

	iRepublic = GC.getInfoTypeForString("CIVIC_REPUBLIC")
	if CyPlayer.canDoCivics(iRepublic):
		return TRNSLTR.getText("TXT_KEY_EVENT_CHANGE_CIVICS", (INFO.getTextKey("CIVIC_", iRepublic),))
...
def getHelpSuperVirus3(argsList):
	...
	if iSmartMedicine > 0:
		if not CyCity.hasBonus(iSmartMedicine):
			szHelp += "\n" + TRNSLTR.getText("TXT_KEY_BUILDINGHELP_REQUIRES_STRING", (INFO.getTextKey("BONUS_", iSmartMedicine),))
	...
	if iSmartDrugs > 0 and not GC.getTeam(CyPlayer.getTeam()).isHasTech(iSmartDrugs):
		szHelp += "\n" + TRNSLTR.getText("TXT_KEY_BUILDINGHELP_REQUIRES_STRING", (INFO.getTextKey("TECH_", iSmartDrugs),))
...
def TriggerSuperVirus4(argsList):
  ...
  eventCity.changeOccupation(iChangePopulation)
  eventCity.changeEventAnger(iChangePopulation)

  eventCity.setBuilding(GC.getInfoTypeForString("BUILDING_HOSPITAL"), False)
...
def triggerNewWorldCities(argsList):
	...
	while iNeededCities > 0:
		...
		if iEvent == GC.getInfoTypeForString("EVENT_NEW_WORLD_2"):
			CyCity.setBuilding(GC.getInfoTypeForString("BUILDING_WALLS"), True)
			CyCity.setBuilding(GC.getInfoTypeForString("BUILDING_GRANARY"), True)
			CyCity.setBuilding(GC.getInfoTypeForString("BUILDING_FORGE"), True)
		else:
			CyCity.setBuilding(GC.getInfoTypeForString("BUILDING_WALLS"), True)
			CyCity.setBuilding(GC.getInfoTypeForString("BUILDING_BARRACKS"), True)
			CyCity.setBuilding(GC.getInfoTypeForString("BUILDING_GRANARY"), True)
			CyCity.setBuilding(GC.getInfoTypeForString("BUILDING_FORGE"), True)
			CyCity.setBuilding(GC.getInfoTypeForString("BUILDING_MARKET"), True)
			CyCity.setBuilding(GC.getInfoTypeForString("BUILDING_HARBOR"), True)
			CyCity.setBuilding(GC.getInfoTypeForString("BUILDING_LIGHTHOUSE"), True)
			CyCity.setBuilding(GC.getInfoTypeForString("BUILDING_FISHERMANS_HUT"), True)
		iNeededCities -= 1
...
def getHelpNewWorld(argsList):
	...
	if argsList[0] == GC.getInfoTypeForString("EVENT_NEW_WORLD_2"):
		...
		if iNavigation > -1 and not GC.getTeam(GC.getPlayer(argsList[1].ePlayer).getTeam()).isHasTech(iNavigation):
			szHelp += "\n" + TRNSLTR.getText("TXT_KEY_BUILDINGHELP_REQUIRES_STRING", (INFO.getTextKey("TECH_", iNavigation),))
	return szHelp
	...
	for i in xrange(len(listAffectedPlots)):
		if len(listAffectedPlots) > 0:
			...
			for pPlotUnit in plotX.units():
				if pPlotUnit.getRead()[UnitReadKind.UNIT_READ_DAMAGE] < 50:
					pPlotUnit.setDamage(50, False)
				elif pPlotUnit.getRead()[UnitReadKind.UNIT_READ_DAMAGE] < 75:
					pPlotUnit.setDamage(75, False)
				elif pPlotUnit.getRead()[UnitReadKind.UNIT_READ_DAMAGE] < 90:
...
def doVolcanoPlot(pPlot):
	...
	for pPlotUnit in pPlot.units():
		if pPlotUnit.getRead()[UnitReadKind.UNIT_READ_DAMAGE] < 90: pPlotUnit.setDamage(90, False)
...
def doMinorFire(argsList):
	...
	if iBurnBuilding != -1:
		...
		CyCity.setBuilding(iBurnBuilding, False)
			...
			if pCity.hasBuilding(iBuildingLoop):
				...
				for iCorpLoop in xrange(GC.getNumCorporationInfos()):
					if iRequiredCorporation == iCorpLoop:
						pCity.setBuilding(iBuildingLoop, False)
