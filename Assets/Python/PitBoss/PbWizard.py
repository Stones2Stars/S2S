...
class ModSelectPage(wx.wizard.PyWizardPage):
	def __init__(self, parent):
		...
		self.rbs.append(wx.RadioButton(modPanel, -1, localText.getText("TXT_KEY_MAIN_MENU_NONE", ()), wx.DefaultPosition, wx.DefaultSize, wx.RB_GROUP))
		...
		for index in xrange(PB.getNumMods()):
			self.rbs.append(wx.RadioButton(modPanel, -1, PB.getModAt(index), wx.DefaultPosition, wx.DefaultSize))
...
class SMTPLoginPage(wx.wizard.WizardPageSimple):
	def __init__(self, parent):
		...
		sizer.AddMany([hostLbl, self.host, usernameLbl, self.username, passwordLbl, self.password, emailLbl, self.email])
...
class NetSelectPage(wx.wizard.PyWizardPage):
	def __init__(self, parent):
		...
		sizer = wx.BoxSizer(wx.VERTICAL)
		self.rb = wx.RadioBox(self, -1, localText.getText("TXT_KEY_PITBOSS_SELECT_NETWORK", ()), wx.DefaultPosition, wx.DefaultSize, selections, 1, wx.RA_SPECIFY_COLS)
...
class LoginPage(wx.wizard.WizardPageSimple):
	...
	def patchAvailable(self, patchName, patchUrl):
		...
		# Put up a dialog
		dlg = wx.MessageDialog(self, localText.getText("TXT_KEY_PITBOSS_PATCH_REQUIRED_DESC", ()), localText.getText("TXT_KEY_PITBOSS_PATCH_REQUIRED_TITLE", ()), wx.OK|wx.CANCEL|wx.ICON_EXCLAMATION)
	...
	def OnPageChanging(self, event):
		...
		if event.GetDirection():
			...
			if not PB.login(self.username.GetValue(), self.password.GetValue()):
				...
				msg = wx.MessageBox((localText.getText("TXT_KEY_PITBOSS_LOGIN_FAILED", ())), (localText.getText("TXT_KEY_PITBOSS_LOGIN_ERROR", ())), wx.ICON_ERROR)
...
class LoadSelectPage(wx.wizard.PyWizardPage):
	def __init__(self, parent):
		...
		sizer = wx.BoxSizer(wx.VERTICAL)
		self.rb = wx.RadioBox(self, -1, (localText.getText("TXT_KEY_PITBOSS_SELECT_INIT", ())), wx.DefaultPosition, wx.DefaultSize, selections, 1, wx.RA_SPECIFY_COLS)
	...
	def patchAvailable(self, patchName, patchUrl):
		...
		# Put up a dialog
		dlg = wx.MessageDialog(self, localText.getText("TXT_KEY_PITBOSS_PATCH_AVAILABLE_DESC", ()), localText.getText("TXT_KEY_PITBOSS_PATCH_AVAILABLE_TITLE", ()), wx.YES_NO|wx.ICON_QUESTION)
	...
	def patchComplete(self):
		...
		# Put up a dialog
		dlg = wx.MessageDialog(self, localText.getText("TXT_KEY_PITBOSS_PATCH_COMPLETE_DESC", ()), localText.getText("TXT_KEY_PITBOSS_PATCH_COMPLETE_TITLE", ()), wx.OK|wx.CANCEL|wx.ICON_EXCLAMATION)
	...
	def OnPageChanging(self, event):
		...
		if event.GetDirection():
			...
			if self.rb.GetSelection() == 2:
				...
				if self.rb.GetSelection() == 0 and PB.getNumMapScripts() == 0:
					# New game - check maps
				...
				if self.rb.GetSelection() == 1 and PB.getNumScenarios() == 0:
				...
				dlg = wx.TextEntryDialog(self, localText.getText("TXT_KEY_PITBOSS_NAME_GAME_DESC", ()), localText.getText("TXT_KEY_PITBOSS_NAME_GAME_TITLE", ()))
				...
				if dlg.ShowModal() == wx.ID_OK:
					...
					if gamename:
						...
						bOK = not bPublic
						if bPublic:
							dlg = wx.TextEntryDialog(self, localText.getText("TXT_KEY_PITBOSS_PWD_GAME_DESC", ()), localText.getText("TXT_KEY_PITBOSS_PWD_GAME_TITLE", ()))
						...
						if bOK:
							...
							event.Veto()
					else: # Malicious user didn't enter a gamename...
						event.Veto()
				else: # User hit cancel
					event.Veto()
				dlg.Destroy()
...
class ScenarioSelectPage(wx.wizard.WizardPageSimple):
	def __init__(self, parent):
		...
		for index in xrange(PB.getNumScenarios()):
			...
			if not index:
				self.rbs.append(wx.RadioButton(scenarioPanel, -1, PB.getScenarioAt(index), wx.DefaultPosition, wx.DefaultSize, wx.RB_GROUP))
			else:
				self.rbs.append(wx.RadioButton(scenarioPanel, -1, PB.getScenarioAt(index), wx.DefaultPosition, wx.DefaultSize))
		...
		if event.GetDirection():
			...
			if PB.loadScenarioInfo(PB.getScenarioAt(iSelection)):
				...
				if not PB.host(bPublic, bScenario):
					...
					PB.reset()
					event.Veto()
			else: # Loading the scenario failed
...
class StagingPage(wx.wizard.WizardPageSimple):
	def __init__(self, parent):
		...
		mapNameList = []
		rowNum = 0
		for rowNum in xrange(PB.getNumMapScripts()):
			mapNameList.append((PB.getMapNameAt(rowNum)))

		# World size
		sizeList = []
		rowNum = 0
		for rowNum in xrange(PB.getNumSizes()):
			sizeList.append((PB.getSizeAt(rowNum)))

		# Climate
		climateList = []
		rowNum = 0
		for rowNum in xrange(PB.getNumClimates()):
			climateList.append((PB.getClimateAt(rowNum)))

		# Sealevel
		seaLevelList = []
		rowNum = 0
		for rowNum in xrange(PB.getNumSeaLevels()):
			seaLevelList.append((PB.getSeaLevelAt(rowNum)))

		# Era
		eraList = []
		rowNum = 0
		for rowNum in xrange(PB.getNumEras()):
			eraList.append((PB.getEraAt(rowNum)))

		# Game speed
		speedList = []
		rowNum = 0
		for rowNum in xrange(PB.getNumSpeeds()):
		...
		rowNum = 0
		for rowNum in xrange(PB.getNumCivs()):
		...
		diffList = []
		rowNum = 0
		for rowNum in xrange(PB.getNumHandicaps()):
	...
	def OnMaxTurnsEntered(self, event):
		...
		if self.maxTurnsEdit.GetValue():
			...
			if not self.IsNumericString(self.maxTurnsEdit.GetValue()):
				...
				dlg = wx.MessageDialog(self, localText.getText("TXT_KEY_PITBOSS_MAXTURN_ERROR_DESC", ()), localText.getText("TXT_KEY_PITBOSS_MAXTURN_ERROR_TITLE", ()), wx.OK|wx.ICON_EXCLAMATION)
	...
	def OnCityEliminationEntered(self, event):
		...
		if self.cityEliminationEdit.GetValue():
			...
			if not self.IsNumericString(self.cityEliminationEdit.GetValue()):
				# It's not - lay the smack down
				dlg = wx.MessageDialog(self, localText.getText("TXT_KEY_PITBOSS_CITYELIMINATION_ERROR_DESC", ()), localText.getText("TXT_KEY_PITBOSS_CITYELIMINATION_ERROR_TITLE", ()), wx.OK|wx.ICON_EXCLAMATION)
	...
	def OnTurnTimeEntered(self, event):
		...
		if self.turnTimerEdit.GetValue():
			...
			if not self.IsNumericString(self.turnTimerEdit.GetValue()):
				# It's not - lay the smack down
				dlg = wx.MessageDialog(self, localText.getText("TXT_KEY_PITBOSS_TURNTIMER_ERROR_DESC", ()), localText.getText("TXT_KEY_PITBOSS_TURNTIMER_ERROR_TITLE", ()), wx.OK|wx.ICON_EXCLAMATION)
	...
	def OnPlayerChoice(self, event):
		...
		PB.playerParamChanged(
			iRow, iValue, self.civArray[iRow].GetSelection() - 1, self.teamArray[iRow].GetSelection(), self.diffArray[iRow].GetSelection(),
	...
	def refreshWhoBox(self, iRow, iWho):
		...
		else: # Set the current player name with the new one
	...
	def refreshCustomMapOptions(self, szMapName):
		...
		for i in xrange(len(self.customItemSizerArray)):
			...
			success = currentSizer.Remove(1)
			success = currentSizer.Remove(0)
