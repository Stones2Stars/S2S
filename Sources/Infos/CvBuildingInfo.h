...
// The sane-info GROUPED SCALAR vocabulary (patterns.md § coherent surface) -- the general family kind-enums
// (DefenseKind / MaintenanceKind / ... / WellbeingKind) every grouped info reads through, lifted to a shared
// header so civic/tech/... reuse the SAME vocabulary rather than minting divergent per-info enums. BuildingScalarKind
// is the building's own scalar catch-all (an info-specific catch-all stays with its info; the families are shared).
#include "CvInfoScalarKinds.h"
