...
//
//	CvTerrainInfo -- the JSON-populated real poco for TERRAINS (the CvXInfo replacement the engine reads normally).
...
public:
	...
	DllExport const CvArtInfoTerrain* getArtInfo() const;   // EXE map-gen art (merged from the removed shim leaf)
...
private:
	...
	int m_iZobristValue;               // map-hash drawn from the synced RNG in the ctor (exact legacy behavior, OOS-load-bearing); CvPlot map hash
