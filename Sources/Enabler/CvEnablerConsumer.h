...
//	(docs/specs/enabler.md (enabler and cascade are two separate systems)) and each owns its own IEventConsumer, registered independently on the spine --
...
//	edges straight off the loaded info objects (docs/spine.md §5 (the load reseed)). It therefore does NOT suppress during the
