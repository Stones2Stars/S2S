...
public:
	...
	bool canAnimalIgnoresBorders() const { return skill("animalIgnoresBorders"); }  // skills.animalIgnoresBorders
	...
	const IDValueMap<UnitTypes, int, -1>& getFlankingStrikeUnits() const { return m_flankingStrikeUnits; }  // GAP: FlankingStrikes (by-unit) not in curator VS_KEYED
	...
	int getReligionSpreads(int /*i*/) const { return 0; }        // GAP: grants.religionSpreads is a flat list (per-religion int dropped)
	int getCorporationSpreads(int /*i*/) const { return 0; }     // GAP: grants.corporationSpreads is a flat list (per-corp int dropped)
	int getBaseCargoVolume() const { return 0; }                 // GAP: SM cargo volume not curated (size-matters pass deferred)
	...
	int getNumGroupSpawnUnitCombatTypes() const { return 0; }     // GAP: grants.groupSpawn is a flat list (chance/title dropped)
	const GroupSpawnUnitCombat& getGroupSpawnUnitCombatType(int /*i*/) const { return m_emptyGroupSpawnUnitCombat; }
	...
	int getEraInfo() const { return -1; }                        // runtime-derived from the prereq tech's era (not stored)
	...
	int getAnimalIgnoresBorders() const { return canAnimalIgnoresBorders() ? 1 : 0; }  // curator keeps only the bool (magnitude dropped)
	...
	int getTotalModifiedCombatStrength100(const bool /*bSizeMatters*/) const { return 0; }  // runtime combat calc
	...
	bool isNoRevealMap() const { return false; }                 // GAP: bNoRevealMap not curated
	bool canMergeSplit() const { return false; }                 // GAP: bCanMergeSplit not curated
	bool isMilitaryProduction() const { return false; }          // curator DROPS bMilitaryProduction (military-ness = the `military` tag)
	bool isMilitaryHappiness() const { return false; }           // curator DROPS bMilitaryHappiness
	bool isMilitarySupport() const { return false; }             // curator DROPS bMilitarySupport (IS_MILITARY -> `military` tag)
	...
	void setPowerValue(int /*iNewValue*/) {}                     // legacy runtime setter (no-op; value comes from JSON)
	...
	bool canAcquireExperience() const { return true; }           // default: units gain XP unless explicitly barred (not curated)
