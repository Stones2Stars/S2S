...
public:
	...
	// files, served by the shim's DllExport getArtInfo() at runtime -- NOT feature-curator output, so the poco cannot
	// source them. getArtInfo()->getNumVarieties() is what the on-map callers already use; this poco getter is Python-pedia only.
	int getNumVarieties() const { return 0; }
	bool canBeSecondary() const { return true; }
