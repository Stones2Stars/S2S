...
void CvJsonImprovementInfo::mapFrom(const picojson::value& entity)
{
	...
	if (const picojson::object* io = jsonChildObj(o, "identity"))
	{
		...
		// isImprovementBonusTrade already reads the universal lynchpin above; the per-bonus RNG values live here.
...
TechTypes CvJsonImprovementInfo::getPrereqTech() const
{
	if (m_requires.build)
...
bool CvJsonImprovementInfo::isRequiresFeature() const    { return condHasPredicate(m_requires.build, CASC_PRED_HAS_FEATURE, -1); }
bool CvJsonImprovementInfo::isRequiresFlatlands() const  { return condHasPredicate(m_requires.build, CASC_PRED_IS_FLATLANDS, -1); }
bool CvJsonImprovementInfo::isRequiresIrrigation() const { return condHasPredicate(m_requires.build, CASC_PRED_HAS_IRRIGATION, -1); }
bool CvJsonImprovementInfo::isWaterImprovement() const   { return condHasPredicate(m_requires.build, CASC_PRED_IS_WATER, -1); }

bool CvJsonImprovementInfo::isCanMoveSeaUnits() const
{
	return condHasPredicate(m_requires.build, CASC_PRED_IS_LAND, -1) && condHasPredicate(m_requires.build, CASC_PRED_HAS_COAST, -1);
}

bool CvJsonImprovementInfo::isPeakImprovement() const
{
	int d = 0, n = 0; condCountPred(m_requires.build, CASC_PRED_HAS_PEAK, false, d, n);
	return d >= 1;
}

bool CvJsonImprovementInfo::isPeakMakesValid() const
{
	int d = 0, n = 0; condCountPred(m_requires.build, CASC_PRED_HAS_PEAK, false, d, n);
	return n > 0 || d >= 2;
}

bool CvJsonImprovementInfo::isRiverSideMakesValid() const
{
	int d = 0, n = 0; condCountPred(m_requires.build, CASC_PRED_HAS_RIVER, false, d, n);
	return n > 0 || d > (m_bRequiresRiverSide ? 1 : 0);
}

bool CvJsonImprovementInfo::isNoFreshWater() const
{
...
bool CvJsonImprovementInfo::isHillsMakesValid() const      { return condHasPredicate(m_requires.build, CASC_PRED_HAS_HILLS, -1); }
bool CvJsonImprovementInfo::isFreshWaterMakesValid() const { return condHasPredicate(m_requires.build, CASC_PRED_HAS_FRESHWATER, -1); }
bool CvJsonImprovementInfo::getTerrainMakesValid(int i) const { return condHasPredicate(m_requires.build, CASC_PRED_HAS_TERRAIN, i); }
bool CvJsonImprovementInfo::getFeatureMakesValid(int i) const { return condHasPredicate(m_requires.build, CASC_PRED_HAS_FEATURE, i); }
bool CvJsonImprovementInfo::isImprovementBonusMakesValid(int i) const { return condHasBonusPresence(m_requires.build, i); }

bool CvJsonImprovementInfo::isAlternativeImprovementUpgradeType(int i) const
{
...
bool CvJsonImprovementInfo::isFeatureChangeType(int i) const
{
...
void CvJsonImprovementInfo::readConditionalYields(const picojson::value& entity)
{
	if (!entity.is<picojson::object>()) return;
	const picojson::object& o = entity.get<picojson::object>();
...
void CvJsonImprovementInfo::readPrereqNatureYield(const picojson::value& entity)
{
	if (!entity.is<picojson::object>()) return;
	const picojson::object& o = entity.get<picojson::object>();
...
int CvJsonImprovementInfo::getImprovementBonusYield(int i, int j) const
{
...
int CvJsonImprovementInfo::getTechYieldChanges(int i, int j) const
{
	if (j < 0 || j >= NUM_YIELD_TYPES) return 0;
...
int* CvJsonImprovementInfo::getTechYieldChangesArray(int i) const
{
	std::map<int, std::vector<int> >::const_iterator it = m_techYieldChanges.find(i);
