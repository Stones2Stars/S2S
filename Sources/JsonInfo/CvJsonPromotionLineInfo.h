...
//	CvJsonPromotionInfo::getPromotionLine), NOT JSON data. No cascade here.
//
...
{
public:
	CvJsonPromotionLineInfo() : m_bBuildUp(false), m_eTechPrereq(NO_TECH), m_eObsoleteTech(NO_TECH) {}
	...
	// condition tree in mapFrom (the GAMEOPTION_ presence-atom ids -- the exact CvJsonPromotionInfo idiom).
	...
	// unitcombat-prereq surface (consumed by CvJsonPromotionInfo::getQualifiedUnitCombatType reconstruction):
	...
	// the member promotions -- a RUNTIME reverse index (rebuilt post-load from CvJsonPromotionInfo::getPromotionLine),
