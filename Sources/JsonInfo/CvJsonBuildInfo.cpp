...
bool CvJsonBuildInfo::isFeatureRemove(FeatureTypes e) const
{
	const FeatureStruct* p = findFeatureStruct(m_aFeatureStructs, e);
	return p ? p->bRemove : false;
}

TechTypes CvJsonBuildInfo::getFeatureTech(FeatureTypes e) const
{
	const FeatureStruct* p = findFeatureStruct(m_aFeatureStructs, e);
	return p ? p->ePrereqTech : NO_TECH;
}

int CvJsonBuildInfo::getFeatureTime(FeatureTypes e) const
{
	const FeatureStruct* p = findFeatureStruct(m_aFeatureStructs, e);
	return p ? p->iTime : 0;
}

int CvJsonBuildInfo::getFeatureProduction(FeatureTypes e) const
{
	const FeatureStruct* p = findFeatureStruct(m_aFeatureStructs, e);
...
{
	CvJsonInfo::mapFrom(entity);   // core reading + availability model (populates m_requires, incl. requires.build)
