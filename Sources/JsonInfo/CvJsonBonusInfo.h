...
public:
	...
	// runtime GameFont glyph (assigned post-load by CvGameTextMgr's symbol pass -- NOT JSON; mirrors CvJsonReligionInfo)
	...
	const CvPropertyManipulators* getPropertyManipulators() const { return &m_PropertyManipulators; }  // no bonus authors a PropertyManipulators element (0 across source XML + JSON); empty is faithful. Property-engine migration is a separate deferred pass (mirrors CvJsonCorporationInfo).
...
private:
	...
	CvPropertyManipulators m_PropertyManipulators;   // empty -- no bonus authors PropertyManipulators (0 across source XML + JSON); property-engine migration deferred (mirrors CvJsonCorporationInfo)
