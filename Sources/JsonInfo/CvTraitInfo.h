...
//
//	CvTraitInfo -- the per-type cascade info for TRAITS (the COMMON base of the two trait sets). Composes the
...
public:
	...
	// DEAD on the poco -- validity depends on live GAME OPTIONS, which a pure-data poco must not read
	// ([DEC-json-not-cascade]). The logic moved to the consumer: CvGame::isTraitValid(eTrait, bGameStart) (owner
	// 2026-07-11). All callers use that; this stub is retained only to keep the archived getter surface complete.
	...
	// Prereqs are REVERSE-MAPPED at load (CvGlobals doPostLoadCaching): the curator inverts each trait's prereqs onto
	// the prereq entity's enables, split by bucket to keep AND vs OR distinct -- the prereq TRAIT carries
	// enables.traitsAnd (single AND) / enables.traitsOr (the OR pair); the prereq TECH carries enables.traits. The
	// load pass reconstructs the forward getters the engine + pedia read (CvPlayer.cpp:29492, CvGameTextMgr.cpp:6214).
	TraitTypes getPrereqTrait() const { return m_ePrereqTrait; }
	TraitTypes getPrereqOrTrait1() const { return m_ePrereqOrTrait1; }
	TraitTypes getPrereqOrTrait2() const { return m_ePrereqOrTrait2; }
	TechTypes getPrereqTech() const { return m_ePrereqTech; }
	void setPrereqTrait(TraitTypes e) { m_ePrereqTrait = e; }                 // load-time reverse-index writers
	void addPrereqOrTrait(TraitTypes e) { if (m_ePrereqOrTrait1 == NO_TRAIT) m_ePrereqOrTrait1 = e; else if (m_ePrereqOrTrait2 == NO_TRAIT) m_ePrereqOrTrait2 = e; }
	void setPrereqTech(TechTypes e) { m_ePrereqTech = e; }
	...
	// Materialize the per-specialist array view (recompute-on-read into a game-thread buffer; the consumer --
	// setYield/CommerceChangeHelp -- reads it immediately, before the next call overwrites). Reuses the per-index getters.
	int* getSpecialistYieldChangeArray(int i) const
	{
		for (int j = 0; j < NUM_YIELD_TYPES; ++j) m_aiSpecYieldBuf[j] = getSpecialistYieldChange(i, j);
		return m_aiSpecYieldBuf;
	}
	int* getSpecialistCommerceChangeArray(int i) const
	{
		for (int j = 0; j < NUM_COMMERCE_TYPES; ++j) m_aiSpecCommerceBuf[j] = getSpecialistCommerceChange(i, j);
		return m_aiSpecCommerceBuf;
	}
...
private:
	...
	TraitTypes m_ePrereqTrait;         // reverse-mapped at load (enables.traitsAnd); getPrereqTrait
	TraitTypes m_ePrereqOrTrait1;      // reverse-mapped at load (enables.traitsOr, slot 1)
	TraitTypes m_ePrereqOrTrait2;      // reverse-mapped at load (enables.traitsOr, slot 2)
	TechTypes m_ePrereqTech;           // reverse-mapped at load (prereq tech's enables.traits); getPrereqTech
