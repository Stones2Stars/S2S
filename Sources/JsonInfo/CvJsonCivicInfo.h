...
//	meaning with two grantors (a civic enacts them, a trait grants them permanently), so CvJsonTraitInfo composes
...
//	    UN-conditioned, non-`per`, un-qualified entries at the curated address (the CvJsonBuildingInfo sumUnconditioned
...
public:
	...
	// getCityLimit: game-state FORMULA getter (isOption + world-size scaling); raw datum at happiness.empire.cityLimit.flat.
	// A pure-data poco must not read GC.getGame().isOption ([DEC-json-not-cascade]); 0 == archive when the option is OFF.
