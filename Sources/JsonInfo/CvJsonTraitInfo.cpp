//
//	CvJsonTraitInfo -- ctor, mapFrom, and the mirrored-getter support. Grounded field-by-field against
...
//	    (CvJsonCivicInfo's proven sumUnconditioned approach; jsonFamVal alone returns 0 on such a list -- the bug fixed).
...
//	    target's own GC type string (CvJsonCivicInfo's sumKeyed template).
...
//
//	Inherited by CvJsonSimpleTraitInfo / CvJsonComplexTraitInfo. See the header for the exact per-getter mapping.
//
...
#include "CvJsonSpecialistInfo.h"   // GC.getSpecialistInfo(i).getType() -- the 2D specialist address
#include "Infos/CvImprovementInfo.h" // GC.getImprovementInfo(i).getType() -- the 2D improvement address
...
CvJsonTraitInfo::CvJsonTraitInfo()
...
{
	CvJsonInfo::mapFrom(entity);   // core reading + section dispatch (edges/grants/modifiers/policies)
...
//     own GC type string (CvJsonCivicInfo's sumKeyed template; curate_trait.py KEYED Specialist*/ImprovementYieldChanges) ---
int CvJsonTraitInfo::getSpecialistYieldChange(int i, int j) const
{
...
int CvJsonTraitInfo::getSpecialistCommerceChange(int i, int j) const
{
...
int CvJsonTraitInfo::getImprovementYieldChange(int i, int j) const
{
...
bool CvJsonTraitInfo::isAnySpecialistYieldChanges() const
{
	for (int j = 0; j < NUM_YIELD_TYPES; ++j)
		if (traitHasPrefix(getModifiers(), std::string(YIELD_FAM[j]) + ".empire.specialists.")) return true;
	return false;
}
bool CvJsonTraitInfo::isAnySpecialistCommerceChanges() const
{
...
const IDValueMap<BuildingTypes, int>::filtered CvJsonTraitInfo::getBuildingHappinessModifiersFiltered() const
{ return jsonEmptyFiltered(m_aBuildingHappinessModifiers); }
const IDValueMap<ImprovementTypes, int>::filtered CvJsonTraitInfo::getImprovementUpgradeModifiers() const
{ return jsonEmptyFiltered(m_aImprovementUpgradeModifierTypes); }
const IDValueMap<BuildTypes, int>::filtered CvJsonTraitInfo::getBuildWorkerSpeedModifiers() const
{ return jsonEmptyFiltered(m_aBuildWorkerSpeedModifierTypes); }
const IDValueMap<DomainTypes, int>::filtered CvJsonTraitInfo::getDomainFreeExperience() const
{ return jsonEmptyFiltered(m_aDomainFreeExperiences); }
const IDValueMap<DomainTypes, int>::filtered CvJsonTraitInfo::getDomainProductionModifiers() const
{ return jsonEmptyFiltered(m_aDomainProductionModifiers); }
const IDValueMap<TechTypes, int>::filtered CvJsonTraitInfo::getTechResearchModifiers() const
{ return jsonEmptyFiltered(m_aTechResearchModifiers); }
const IDValueMap<SpecialBuildingTypes, int>::filtered CvJsonTraitInfo::getSpecialBuildingProductionModifiers() const
{ return jsonEmptyFiltered(m_aSpecialBuildingProductionModifiers); }
const IDValueMap<BonusTypes, int>::filtered CvJsonTraitInfo::getBonusHappinessChanges() const
