...
public:
	CvJsonBuildingInfo()
		: notConstructible(false), governmentCenter(false), forceNoPrereqScaling(false),
		  shrineReligion(-1), corpHQ(-1), religion(-1), freeStartEra(-1), conquestProbability(0), voteSourceType(-1),
		  autoBuild(false),
	...
	{ for (int i = 0; i < NUM_YIELD_TYPES; ++i) m_powerYieldModifier[i] = 0; }
	...
	bool isProvidesFreshWater() const      { return getAttributes()->has("providesFreshWater"); }
	bool isForceAllTradeRoutes() const     { return getAttributes()->has("forceAllTradeRoutes"); }
	bool isForceNoPrereqScaling() const    { return forceNoPrereqScaling; }
	bool isZoneOfControl() const           { return getAttributes()->has("zoneOfControl"); }
	bool isProtectedCulture() const        { return getAttributes()->has("protectedCulture"); }
	bool isNoLimit() const                 { return m_bNoLimit; }
	bool isTeamShare() const               { return getAttributes()->has("teamShare"); }
	bool isAutoBuild() const               { return autoBuild; }
	bool isOrbital() const                 { return getAttributes()->has("orbital"); }
	bool isOrbitalInfrastructure() const   { return getAttributes()->has("orbitalInfrastructure"); }
	bool isAreaBorderObstacle() const      { return getAttributes()->has("borderObstacle"); }
	bool isForceTeamVoteEligible() const   { return false; }   // STUB curator emits it as an `enables.votes` marker string "FORCE_TEAM_ELIGIBLE" (not a resolvable INFOTYPE); CvJsonEdges drops it -> unrecoverable as authored (curator/reader fix owed)
	bool isCapital() const                 { return getAttributes()->has("capital"); }
	bool isGovernmentCenter() const        { return governmentCenter; }
	bool isMapCentering() const            { return getAttributes()->has("mapCentering"); }
	bool isNoUnhappiness() const           { return getAttributes()->has("noUnhappiness"); }
	bool isNoUnhealthyPopulation() const   { return getAttributes()->has("noUnhealthyPopulation"); }
	bool isBuildingOnlyHealthy() const     { return getAttributes()->has("buildingOnlyHealthy"); }
	bool isNeverCapture() const            { return getAttributes()->has("destroyedOnCapture"); }  // curator RENAMED bNeverCapture -> attributes.destroyedOnCapture (owner ruling 2026-07-01)
	bool isNukeImmune() const              { return getAttributes()->has("nukeImmune"); }
	...
	bool isPrereqWar() const               { return false; }   // CURATOR-GAP: bPrereqWar listed in REQUIRES_TAGS but emitted by NO curator code
	bool isRequiresActiveCivics() const    { return false; }   // CURATOR-GAP: bRequiresActiveCivics listed but not emitted (civic prereqs are unconditionally on operate)
	...
	bool isDamageAllAttackers() const      { return false; }   // CURATOR-GAP: folded into defense.city.counterDamage (bespoke object, absence of `units` selector = all attackers -- not separately queryable here)
	bool isDamageAttackerCapable() const   { return false; }   // CURATOR-GAP: derived from the counter-damage fold (see above)
	...
	int getExtraPlayerInstances() const      { return 0; }   // STUB folded into allowed.empire (curator); the extra component alone is not recoverable
	...
	// --- RUNTIME (set post-load, NOT JSON; mirrors CvJsonCorporationInfo) ---
	...
	int getRiverPlotYieldChange(int /*i*/) const    { return 0; }   // CURATOR-GAP: RiverPlotYieldChanges folded into a plots-target modifier (HAS_RIVER), not the keyed legacy array
	...
	// STUB SCALE AMBIGUITY (owner attention, not guessed): the curator DE-scales the yield/commerce `perPopulation`
	// leaves by /100 (YIELD_FAMILIES path) yet does NOT de-scale the happiness/health perPopulation leaves
	// (SCALAR_FAMILIES path), and docs/specs/curators/fixed-point-and-scales.md §4c says per-pop is "×1 human, do NOT
	// de-scale" -- three sources disagree on this field's true scale. Computing a value risks the exact 1/pop ->
	// 0.01/pop corruption the doc warns about, so all four per-pop getters stay STUB 0 pending resolution.
	int getYieldPerPopChange(int /*i*/) const        { return 0; }
	int getCommercePerPopChange(int /*i*/) const     { return 0; }
	int getHappinessPercentPerPopulation() const     { return 0; }
	int getHealthPercentPerPopulation() const        { return 0; }
	...
	int getMaxPopulationAllowed() const    { return 0; }    // CURATOR-GAP: DROP_DEAD
	...
	const std::vector<BonusTypes>& getConsumptionRelevantBonuses() const { return m_consumptionRelevantBonuses; }  // STUB empty -- load-derived scan not reconstructed
	const IDValueMap<BonusTypes, int>& getFreeBonuses() const                { return m_freeBonuses; }             // STUB empty -- ExtraFreeBonuses is grants-shaped, not curated as a value map
	...
	const CvPropertyManipulators* getPropertyManipulators() const { return &m_PropertyManipulators; } // STUB empty -- XML-era manipulator data deferred (mirrors CvJsonCorporationInfo)
	...
	// boost::python come from the PCH). Exact archived signatures so the .def(&CvJsonBuildingInfo::cyGet...) binds. ---
	...
	int getMayDamageAttackingUnitCombatType(int /*i*/) const { return -1; }   // CURATOR-GAP: MayDamageAttackingUnitCombatTypes folded into defense.city.counterDamage.units.unitCombats (a bespoke non-keyed object, not a keyed modifier)
	int getNumMayDamageAttackingUnitCombatTypes() const      { return 0; }
	bool isMayDamageAttackingUnitCombatType(int /*i*/) const { return false; }
...
private:
	...
	int m_iMissionType;             // RUNTIME (set post-load, NOT JSON; mirrors CvJsonCorporationInfo)
