...
//
//	CvProjectInfo -- the JSON real poco for PROJECTS (team-built endeavours: spaceship parts, Manhattan/SDI class,
...
public:
	...
	// archive CvProjectInfo::getVictoryMinThreshold: when the min is 0/unset, FALL BACK to the plain victory threshold
	// (a project with a threshold but no explicit min must not read 0 -- the Space-victory min-project gate).
	int getVictoryMinThreshold(int iVictory) const
	{ const int iMin = mapGet(m_victoryMinThreshold, iVictory); return iMin != 0 ? iMin : getVictoryThreshold(iVictory); }
