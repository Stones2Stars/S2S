//
//	CvJsonUnitInfo::mapFrom -- wires the unit poco's typed members to the curated JSON (Tools/Migration/curate_unit.py).
//	The base CvJsonInfo::mapFrom fills type/identity-text/button + the composed section units (skills/tags/requires/
...
#include "CvJsonUnitCombatInfo.h"   // GC.getUnitCombatInfo(e).getGroupBase() -- the baseGroupRank derivation

CvJsonUnitInfo::CvJsonUnitInfo()
...
const CvArtInfoUnit* CvJsonUnitInfo::getArtInfo(int /*i*/, EraTypes /*eEra*/, UnitArtStyleTypes /*eStyle*/) const
{
...
const char* CvJsonUnitInfo::getButton() const
{
...
int CvJsonUnitInfo::getBaseGroupRank() const
{
...
// ------------------------------------------------------------------------------------------------------------------
void CvJsonUnitInfo::reconstructPrereqs()
{
...
{
	CvJsonInfo::mapFrom(entity);   // core fields + composed units (skills/tags/requires/grants/edges/modifier families)
	if (!entity.is<picojson::object>()) return;
	const picojson::object& o = entity.get<picojson::object>();
	picojson::object::const_iterator it;

	// world.art.icon -- the ART_DEF_* tag getArtInfo resolves through ArtFileMgr (mirrors CvJsonBonusInfo).
