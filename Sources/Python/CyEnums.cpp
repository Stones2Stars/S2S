...
//
//	⚑ ENUM OPERATIONS ARE FIRST CLASS (patterns.md § THE PYTHON READ BOUNDARY), covering RESOLUTION and
//	EXTENSION:
//	  - RESOLUTION by name is `getInfoType` below -- the generalization of the engine's own getInfoTypeForString,
//	    which every script that turns a config string into an id needs.
//	  - EXTENSION is served BY THE PUBLISHED ENUM OBJECTS THEMSELVES. A boost enum is a real Python type, so
//	    BUG's existing mechanism -- construct from an int, setattr a new member onto the type -- mints new
//	    WidgetTypes at runtime with no further engine surface. ⛔ No separate mint API is invented here: a verb
//	    with no caller is machinery for a hypothetical, and the three enums that need extension already have
//	    their path the moment these types exist.
//
//	BOOST: only the `python::` alias, never a bare `boost::` -- the tree carries two Boosts (engine.md).
//
...
// Resolution: an INFOTYPE_NAME ("BUILDING_FORGE", "TECH_POTTERY") to its engine id, -1 when unknown. Assert is
// suppressed because a script legitimately probes for a type that may not exist in this ruleset.
int CyEnums::getInfoType(const std::string& szType) const
{
	return GC.getInfoTypeForString(szType.c_str(), /*hideAssert*/ true);
}

void CyEnums::pythonPublish()
{
	...
	python::enum_<PyIdListSlot>("IdListSlot")
		...
		.value("PYLIST_TRADE_PROVIDING_IMPROVEMENTS", PYLIST_TRADE_PROVIDING_IMPROVEMENTS)
	;
	...
	python::enum_<PyIntrinsicSlot>("IntrinsicSlot")
		...
		.value("PYINT_IS_DISABLED",       PYINT_IS_DISABLED)
		...
		.value("PYINT_UNIT_COMBAT", PYINT_UNIT_COMBAT)
		.value("PYINT_HURRY_GOLD_PER_PRODUCTION", PYINT_HURRY_GOLD_PER_PRODUCTION)
		.value("PYINT_HURRY_PRODUCTION_PER_POPULATION", PYINT_HURRY_PRODUCTION_PER_POPULATION)
		.value("PYINT_HURRY_IS_ANGER", PYINT_HURRY_IS_ANGER)
		.value("PYINT_ESPIONAGE_COST", PYINT_ESPIONAGE_COST)
		.value("PYINT_ESPIONAGE_TARGETS_CITY", PYINT_ESPIONAGE_TARGETS_CITY)
		.value("PYINT_ESPIONAGE_IS_PASSIVE", PYINT_ESPIONAGE_IS_PASSIVE)
		.value("PYINT_ESPIONAGE_TECH_PREREQ", PYINT_ESPIONAGE_TECH_PREREQ)
		.value("PYINT_TECH_REVEAL", PYINT_TECH_REVEAL)
		.value("PYINT_TECH_CITY_TRADE", PYINT_TECH_CITY_TRADE)
		.value("PYINT_TECH_OBSOLETE", PYINT_TECH_OBSOLETE)
		.value("PYINT_MIN_LATITUDE", PYINT_MIN_LATITUDE)
	...
	python::enum_<UnitReadKind>("UnitReadKind")
		.value("UNIT_READ_TYPE", UNIT_READ_TYPE)
		.value("UNIT_READ_DOMAIN", UNIT_READ_DOMAIN)
		.value("UNIT_READ_LEVEL", UNIT_READ_LEVEL)
		.value("UNIT_READ_HP", UNIT_READ_HP)
		.value("UNIT_READ_MAX_HP", UNIT_READ_MAX_HP)
		.value("UNIT_READ_MOVES_LEFT", UNIT_READ_MOVES_LEFT)
		.value("UNIT_READ_BASE_MOVES", UNIT_READ_BASE_MOVES)
		.value("UNIT_READ_EXPERIENCE", UNIT_READ_EXPERIENCE)
		.value("UNIT_READ_EXPERIENCE_NEEDED", UNIT_READ_EXPERIENCE_NEEDED)
		.value("UNIT_READ_HOTKEY_NUMBER", UNIT_READ_HOTKEY_NUMBER)
		.value("UNIT_READ_BASE_COMBAT", UNIT_READ_BASE_COMBAT)
		.value("UNIT_READ_AIR_BASE_COMBAT", UNIT_READ_AIR_BASE_COMBAT)
		.value("UNIT_READ_CONTROL_POINTS", UNIT_READ_CONTROL_POINTS)
		.value("UNIT_READ_CONTROL_POINTS_LEFT", UNIT_READ_CONTROL_POINTS_LEFT)
		.value("UNIT_READ_COMMODORE_CONTROL_POINTS", UNIT_READ_COMMODORE_CONTROL_POINTS)
		.value("UNIT_READ_COMMODORE_CONTROL_POINTS_LEFT", UNIT_READ_COMMODORE_CONTROL_POINTS_LEFT)
		.value("UNIT_READ_ACTIVITY", UNIT_READ_ACTIVITY)
		.value("UNIT_READ_AUTOMATE", UNIT_READ_AUTOMATE)
		.value("UNIT_READ_MISSION", UNIT_READ_MISSION)
		.value("UNIT_READ_MISSION_QUEUE_LENGTH", UNIT_READ_MISSION_QUEUE_LENGTH)
		.value("UNIT_READ_DAMAGE", UNIT_READ_DAMAGE)
		.value("UNIT_READ_FACING_DIRECTION", UNIT_READ_FACING_DIRECTION)
		.value("UNIT_READ_LEADER_UNIT_TYPE", UNIT_READ_LEADER_UNIT_TYPE)
		.value("UNIT_READ_UNIT_AI", UNIT_READ_UNIT_AI)
		.value("UNIT_READ_GROUP_ID", UNIT_READ_GROUP_ID)
		.value("UNIT_READ_CAPTURE_UNIT_TYPE", UNIT_READ_CAPTURE_UNIT_TYPE)
		.value("UNIT_READ_CAPTURE_PROBABILITY", UNIT_READ_CAPTURE_PROBABILITY)
		.value("UNIT_READ_CAPTURE_RESISTANCE", UNIT_READ_CAPTURE_RESISTANCE)
		.value("UNIT_READ_COMBAT_CLASS", UNIT_READ_COMBAT_CLASS)
		.value("UNIT_READ_PILLAGE_CHANGE", UNIT_READ_PILLAGE_CHANGE)
		.value("UNIT_READ_NUKE_RANGE", UNIT_READ_NUKE_RANGE)
		.value("NUM_UNIT_READS", NUM_UNIT_READS)
	;

	python::enum_<UnitFlagKind>("UnitFlagKind")
		.value("UNIT_FLAG_SELECTED", UNIT_FLAG_SELECTED)
		.value("UNIT_FLAG_IN_BATTLE", UNIT_FLAG_IN_BATTLE)
		.value("UNIT_FLAG_WAITING", UNIT_FLAG_WAITING)
		.value("UNIT_FLAG_HURT", UNIT_FLAG_HURT)
		.value("UNIT_FLAG_FORTIFYABLE", UNIT_FLAG_FORTIFYABLE)
		.value("UNIT_FLAG_COMMANDER", UNIT_FLAG_COMMANDER)
		.value("UNIT_FLAG_COMMODORE", UNIT_FLAG_COMMODORE)
		.value("UNIT_FLAG_PROMOTION_READY", UNIT_FLAG_PROMOTION_READY)
		.value("UNIT_FLAG_HAS_MOVED", UNIT_FLAG_HAS_MOVED)
		.value("UNIT_FLAG_CAN_MOVE", UNIT_FLAG_CAN_MOVE)
		.value("UNIT_FLAG_CAN_FIGHT", UNIT_FLAG_CAN_FIGHT)
		.value("UNIT_FLAG_CAN_AIR_ATTACK", UNIT_FLAG_CAN_AIR_ATTACK)
		.value("UNIT_FLAG_MADE_ATTACK", UNIT_FLAG_MADE_ATTACK)
		.value("UNIT_FLAG_DEAD", UNIT_FLAG_DEAD)
		.value("UNIT_FLAG_AUTO_PROMOTING", UNIT_FLAG_AUTO_PROMOTING)
		.value("UNIT_FLAG_AUTO_UPGRADING", UNIT_FLAG_AUTO_UPGRADING)
		.value("NUM_UNIT_FLAGS", NUM_UNIT_FLAGS)
	;

	python::enum_<CityBuildingRead>("CityBuildingRead")
		.value("CITY_BUILDING_HAS", CITY_BUILDING_HAS)
		.value("CITY_BUILDING_ACTIVE", CITY_BUILDING_ACTIVE)
		.value("CITY_BUILDING_HAPPINESS", CITY_BUILDING_HAPPINESS)
		.value("CITY_BUILDING_HEALTH", CITY_BUILDING_HEALTH)
		...
		.value("CITY_BUILDING_YIELD_FIRST", CITY_BUILDING_YIELD_FIRST)
		.value("CITY_BUILDING_COMMERCE_FIRST", CITY_BUILDING_COMMERCE_FIRST)
		.value("NUM_CITY_BUILDING_READS", NUM_CITY_BUILDING_READS)
	;

	python::enum_<CityCountRead>("CityCountRead")
		.value("CITY_COUNT_NATIONAL_WONDERS", CITY_COUNT_NATIONAL_WONDERS)
		.value("CITY_COUNT_MAX_NATIONAL_WONDERS", CITY_COUNT_MAX_NATIONAL_WONDERS)
		.value("CITY_COUNT_WORLD_WONDERS", CITY_COUNT_WORLD_WONDERS)
		.value("CITY_COUNT_MAX_WORLD_WONDERS", CITY_COUNT_MAX_WORLD_WONDERS)
		.value("CITY_COUNT_DEFENSE_MODIFIER", CITY_COUNT_DEFENSE_MODIFIER)
		...
		.value("CITY_COUNT_GAME_TURN_ACQUIRED", CITY_COUNT_GAME_TURN_ACQUIRED)
		.value("NUM_CITY_COUNT_READS", NUM_CITY_COUNT_READS)
	;

	python::enum_<CitySpecialistRead>("CitySpecialistRead")
		.value("CITY_SPECIALIST_COUNT", CITY_SPECIALIST_COUNT)
		.value("CITY_SPECIALIST_FORCED", CITY_SPECIALIST_FORCED)
		.value("CITY_SPECIALIST_FREE", CITY_SPECIALIST_FREE)
		.value("CITY_SPECIALIST_VALID", CITY_SPECIALIST_VALID)
		.value("CITY_SPECIALIST_EMPHASIZED", CITY_SPECIALIST_EMPHASIZED)
		.value("NUM_CITY_SPECIALIST_READS", NUM_CITY_SPECIALIST_READS)
	;

	python::enum_<CityUnitRead>("CityUnitRead")
		.value("CITY_UNIT_PROGRESS", CITY_UNIT_PROGRESS)
		.value("CITY_UNIT_DELAY", CITY_UNIT_DELAY)
		.value("CITY_UNIT_PRODUCTION_DECAY", CITY_UNIT_PRODUCTION_DECAY)
		.value("NUM_CITY_UNIT_READS", NUM_CITY_UNIT_READS)
	;

	python::enum_<CityFlagKind>("CityFlagKind")
		.value("CITY_FLAG_PRODUCING", CITY_FLAG_PRODUCING)
		.value("CITY_FLAG_CITIZENS_AUTOMATED", CITY_FLAG_CITIZENS_AUTOMATED)
		.value("CITY_FLAG_PRODUCTION_AUTOMATED", CITY_FLAG_PRODUCTION_AUTOMATED)
		.value("CITY_FLAG_CAN_CONSCRIPT", CITY_FLAG_CAN_CONSCRIPT)
		.value("CITY_FLAG_DISORDER", CITY_FLAG_DISORDER)
		.value("CITY_FLAG_CAPITAL", CITY_FLAG_CAPITAL)
		.value("CITY_FLAG_GOVERNMENT_CENTER", CITY_FLAG_GOVERNMENT_CENTER)
		.value("CITY_FLAG_POWER", CITY_FLAG_POWER)
		...
		.value("CITY_FLAG_COASTAL", CITY_FLAG_COASTAL)
		.value("NUM_CITY_FLAGS", NUM_CITY_FLAGS)
	;

	python::enum_<CityCountdownKind>("CityCountdownKind")
		.value("COUNTDOWN_HURRY_ANGER", COUNTDOWN_HURRY_ANGER)
		.value("COUNTDOWN_HURRY_ANGER_PERIOD", COUNTDOWN_HURRY_ANGER_PERIOD)
		.value("COUNTDOWN_CONSCRIPT_ANGER", COUNTDOWN_CONSCRIPT_ANGER)
		.value("COUNTDOWN_CONSCRIPT_ANGER_PERIOD", COUNTDOWN_CONSCRIPT_ANGER_PERIOD)
		.value("COUNTDOWN_DEFY_RESOLUTION_ANGER", COUNTDOWN_DEFY_RESOLUTION_ANGER)
		.value("COUNTDOWN_DEFY_RESOLUTION_ANGER_PERIOD", COUNTDOWN_DEFY_RESOLUTION_ANGER_PERIOD)
		.value("COUNTDOWN_HAPPINESS", COUNTDOWN_HAPPINESS)
		.value("COUNTDOWN_OCCUPATION", COUNTDOWN_OCCUPATION)
		.value("COUNTDOWN_ESPIONAGE_HAPPINESS", COUNTDOWN_ESPIONAGE_HAPPINESS)
		.value("COUNTDOWN_ESPIONAGE_HEALTH", COUNTDOWN_ESPIONAGE_HEALTH)
		.value("COUNTDOWN_REV_REQUEST_ANGER", COUNTDOWN_REV_REQUEST_ANGER)
		.value("COUNTDOWN_REV_SUCCESS", COUNTDOWN_REV_SUCCESS)
		.value("NUM_CITY_COUNTDOWN_KINDS", NUM_CITY_COUNTDOWN_KINDS)
	;

	python::enum_<CityRevolutionRead>("CityRevolutionRead")
		.value("CITY_REV_LOCAL_INDEX", CITY_REV_LOCAL_INDEX)
		.value("CITY_REV_INDEX_DISTANCE_MOD", CITY_REV_INDEX_DISTANCE_MOD)
		.value("CITY_REV_COUNTER", CITY_REV_COUNTER)
		.value("CITY_REV_REINFORCEMENT_COUNTER", CITY_REV_REINFORCEMENT_COUNTER)
		.value("CITY_REV_INDEX_PERCENT_ANGER", CITY_REV_INDEX_PERCENT_ANGER)
		.value("NUM_CITY_REV_READS", NUM_CITY_REV_READS)
	;

	python::enum_<CityOrderRead>("CityOrderRead")
		...
		.value("ORDER_READ_PRODUCTION_NEEDED", ORDER_READ_PRODUCTION_NEEDED)
		.value("ORDER_READ_GENERAL_TURNS_LEFT", ORDER_READ_GENERAL_TURNS_LEFT)
		.value("ORDER_READ_TURNS_LEFT", ORDER_READ_TURNS_LEFT)
		.value("ORDER_READ_PRODUCTION_PER_TURN", ORDER_READ_PRODUCTION_PER_TURN)
		.value("ORDER_READ_PRODUCTION_PER_TURN_NO_FOOD", ORDER_READ_PRODUCTION_PER_TURN_NO_FOOD)
	...
	python::enum_<BuildingGrantedKind>("BuildingGrantedKind")
		.value("BUILDING_GRANTED_HAPPINESS", BUILDING_GRANTED_HAPPINESS)
		.value("BUILDING_GRANTED_HEALTH",    BUILDING_GRANTED_HEALTH)
		.value("NUM_BUILDING_GRANTED_KINDS", NUM_BUILDING_GRANTED_KINDS)
	;

	python::enum_<CityYieldTerm>("CityYieldTerm")
		.value("YIELD_TERM_PLOT_BASE", YIELD_TERM_PLOT_BASE)
		.value("YIELD_TERM_PLOT_NATURE", YIELD_TERM_PLOT_NATURE)
		.value("YIELD_TERM_PLOT_IMPROVEMENT", YIELD_TERM_PLOT_IMPROVEMENT)
		.value("YIELD_TERM_PLOT_REST", YIELD_TERM_PLOT_REST)
		.value("YIELD_TERM_TRADE_YIELD", YIELD_TERM_TRADE_YIELD)
		.value("YIELD_TERM_GOLDEN_AGE", YIELD_TERM_GOLDEN_AGE)
		.value("YIELD_TERM_UPPER_FLAT", YIELD_TERM_UPPER_FLAT)
		.value("YIELD_TERM_SPECIALISTS", YIELD_TERM_SPECIALISTS)
		.value("YIELD_TERM_CITY_FLAT", YIELD_TERM_CITY_FLAT)
		.value("YIELD_TERM_PERCENT_SUM", YIELD_TERM_PERCENT_SUM)
		.value("YIELD_TERM_WORKED_PLOTS", YIELD_TERM_WORKED_PLOTS)
		.value("YIELD_TERM_RATE", YIELD_TERM_RATE)
		.value("NUM_CITY_YIELD_TERMS", NUM_CITY_YIELD_TERMS)
	;

	python::enum_<CityCommerceTerm>("CityCommerceTerm")
		.value("COMMERCE_TERM_COMMERCE_YIELD", COMMERCE_TERM_COMMERCE_YIELD)
		.value("COMMERCE_TERM_SLIDER_PERCENT", COMMERCE_TERM_SLIDER_PERCENT)
		.value("COMMERCE_TERM_SHARE", COMMERCE_TERM_SHARE)
		.value("COMMERCE_TERM_PERCENT_SUM", COMMERCE_TERM_PERCENT_SUM)
		.value("COMMERCE_TERM_DEPOSITS", COMMERCE_TERM_DEPOSITS)
		.value("COMMERCE_TERM_PROCESS_CONVERSION", COMMERCE_TERM_PROCESS_CONVERSION)
		.value("COMMERCE_TERM_RATE", COMMERCE_TERM_RATE)
		.value("NUM_CITY_COMMERCE_TERMS", NUM_CITY_COMMERCE_TERMS)
	;
	...
	python::enum_<CityGrowthRead>("CityGrowthRead")
		...
		.value("GROWTH_READ_FOOD_CONSUMPTION", GROWTH_READ_FOOD_CONSUMPTION)
	...
	python::enum_<CityHurryQuote>("CityHurryQuote")
		.value("HURRY_QUOTE_ALLOWED", HURRY_QUOTE_ALLOWED)
		.value("HURRY_QUOTE_POPULATION_COST", HURRY_QUOTE_POPULATION_COST)
		.value("HURRY_QUOTE_PRODUCTION_GAINED", HURRY_QUOTE_PRODUCTION_GAINED)
		.value("HURRY_QUOTE_GOLD_COST", HURRY_QUOTE_GOLD_COST)
		.value("NUM_CITY_HURRY_QUOTES", NUM_CITY_HURRY_QUOTES)
	;
	...
	python::enum_<GameOptionTypes>("GameOptionTypes")
		...
		.value("GAMEOPTION_TECH_CUTTING_EDGE_CUTS", GAMEOPTION_TECH_CUTTING_EDGE_CUTS)
...
// BUG - Sentry Actions - end
	...
	python::enum_<AutomateTypes>("AutomateTypes")
		...
		.value("AUTOMATE_SPREAD", AUTOMATE_SPREAD)
	...
	python::enum_<ActionSubTypes>("ActionSubTypes")
		...
		.value("ACTIONSUBTYPE_HERITAGE", ACTIONSUBTYPE_HERITAGE)
	...
	python::enum_<ModderGameOptionTypes>("ModderGameOptionTypes")
		...
		.value("MODDERGAMEOPTION_NEXT_TRAIT_CULTURE_REQ_PERCENT", MODDERGAMEOPTION_NEXT_TRAIT_CULTURE_REQ_PERCENT)
	...
	// The surface itself: a plain published class carrying the resolution operation, beside the constants above.
	python::class_<CyEnums>("CyEnums")
		.def("getInfoType", &CyEnums::getInfoType)
		;
}
