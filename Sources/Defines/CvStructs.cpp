...
void EventTriggeredData::read(FDataStreamBase* pStream)
{
	...
	// ⛔ ALLOW-MISSING, PAIRED WITH A DROP — neither half works alone (save.md §7). This enum is the record's
	// IDENTITY, so a hard read makes an EVENTTRIGGER Type UNDELETABLE (removing one refused the whole save), while
	// allow-missing on its own would register a live object pointing at nothing. The record's own read() cannot
	// drop itself, so the owner sweeps it after the array streams (CvPlayer::read).
	WRAPPER_READ_CLASS_ENUM_ALLOW_MISSING(wrapper, "EventTriggeredData", REMAPPED_CLASS_TYPE_EVENT_TRIGGERS, (int*)&m_eTrigger);
