...
void CvJsonBuildingInfo::mapFrom(const picojson::value& entity)
{
	...
	if ((it = io.find("shrine")) != io.end() && it->second.is<std::string>())        shrineReligion = cascadeJsonResolveId(it->second.get<std::string>());
	if ((it = io.find("corporationHQ")) != io.end() && it->second.is<std::string>()) corpHQ = cascadeJsonResolveId(it->second.get<std::string>());
	...
	if ((it = io.find("governmentCenter")) != io.end() && it->second.is<bool>())     governmentCenter = it->second.get<bool>();
