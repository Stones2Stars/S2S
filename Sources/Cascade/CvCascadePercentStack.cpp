...
int PercentStack::percentStack(const std::string& channel, const CvCity* pCity, MMBreak& bk)
{
	...
	for (size_t ci = 0; ci < cands.size(); ++ci)
	{
		...
		const CvInfo* d = InfoRepo<CvBuildingInfo>::get().get((int)eB);
		if (d == NULL) continue;
	...
	// (commerce-impacting only). Projects are mapped into InfoRepo<CvProjectInfo> (generic CvInfo deposits) by readJson.
	...
	for (int pj = 0; pj < GC.getNumProjectInfos(); ++pj)
	{
		...
		const CvInfo* d = InfoRepo<CvProjectInfo>::get().get(pj);
