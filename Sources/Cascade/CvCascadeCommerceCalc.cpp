...
#include "CvReligionInfo.h"     // InfoRepo<CvReligionInfo> + getGlobalReligionCommerce (the §2 religion/shrine packages)
#include "CvCorporationInfo.h"  // getHeadquarterCommerce (the §2 corp-HQ package)
#include "CvHeritageInfo.h"     // InfoRepo<CvHeritageInfo> (the §2 player-extra heritage commerce)
...
int CommerceCalc::religion(const std::string& channel, const CvCity* pCity, const CvCascadeEvalCtx& ec)
{
	...
	for (int r = 0; r < GC.getNumReligionInfos(); ++r)
	{
		...
		const CvInfo* d = InfoRepo<CvReligionInfo>::get().get(r);
...
long CommerceCalc::playerExtra(const std::string& channel, const CvPlayer& player, const CvCascadeEvalCtx& ec)
{
	...
	for (int h = 0; h < GC.getNumHeritageInfos(); ++h)
	{
		...
		const CvInfo* d = InfoRepo<CvHeritageInfo>::get().get(h);
	...
	std::map<int, long>& out)
{
	...
	for (int g = 0; g < nB; ++g)
	{
		...
		if (cnt <= 0) continue;
		const CvInfo* dg = InfoRepo<CvBuildingInfo>::get().get(g);
		if (dg == NULL) continue;
...
long CommerceCalc::shrine(const std::string& channel, const CvCity* pCity)
{
	...
	{
		const CvInfo* bd = InfoRepo<CvBuildingInfo>::get().get(*abIt);
		if (bd == NULL) continue;
		const int rel = static_cast<const CvBuildingInfo*>(bd)->shrineReligion;   // cascade identity.shrine FK (self-contained)
		if (rel < 0) continue;
		const CvInfo* rd = InfoRepo<CvReligionInfo>::get().get(rel);
		if (rd == NULL) continue;
		const std::map<std::string, int>& sc = static_cast<const CvReligionInfo*>(rd)->shrineCommerce;
...
long CommerceCalc::corpHQ(const std::string& channel, const CvCity* pCity, const CvCascadeEvalCtx& ec)
{
	...
	for (std::set<int>::const_iterator abIt = ec.activeBuildings->begin(); abIt != ec.activeBuildings->end(); ++abIt)
	{
		const CvInfo* bd = InfoRepo<CvBuildingInfo>::get().get(*abIt);
		if (bd == NULL) continue;
		const int corp = static_cast<const CvBuildingInfo*>(bd)->corpHQ;   // cascade identity.corporationHQ FK (self-contained)
		if (corp < 0) continue;
		const CvInfo* cd = InfoRepo<CvCorporationInfo>::get().get(corp);
		if (cd == NULL) continue;
...
long CommerceCalc::stateReligionPool(const std::string& channel, const CvPlayer& player)
{
	...
	for (int b = 0; b < nB; ++b)
	{
		...
		if (cnt <= 0) continue;
		const CvInfo* bd = InfoRepo<CvBuildingInfo>::get().get(b);
		if (bd == NULL) continue;
		const std::map<std::string, int>& m = static_cast<const CvBuildingInfo*>(bd)->stateReligionCommerce;   // cascade identity (self-contained)
...
int CommerceCalc::stateReligionMatch(const CvCity* pCity, const CvCascadeEvalCtx& ec)
{
	...
	{
		const CvInfo* bd = InfoRepo<CvBuildingInfo>::get().get(*abIt);
		if (bd != NULL && static_cast<const CvBuildingInfo*>(bd)->religion == (int)eState) ++match;   // cascade identity.religion FK
	}
	return match;
}
...
long CommerceCalc::doubleExtra(const std::string& channel, const CvCity* pCity, const CvCascadeEvalCtx& ec)
{
	...
	for (std::set<int>::const_iterator abIt = ec.activeBuildings->begin(); abIt != ec.activeBuildings->end(); ++abIt)
	{
		...
		const CvInfo* bd = InfoRepo<CvBuildingInfo>::get().get(b);
		if (bd == NULL) continue;
		const CvBuildingInfo* bj = static_cast<const CvBuildingInfo*>(bd);
...
int CommerceCalc::corporation(const std::string& channel, const CvCity* pCity, const CvCascadeEvalCtx& ec)
{
	...
	for (int c = 0; c < nC; ++c)
	{
		...
		const CvInfo* cd = InfoRepo<CvCorporationInfo>::get().get(c);   // the corp's cascade data (self-contained, no engine config)
		if (cd == NULL) continue;
		// The corp families are COLLAPSED to typed members (CvCorporationInfo: the uniform HAS_CORPORATION gate --
		...
		const CvCorporationInfo* corp = static_cast<const CvCorporationInfo*>(cd);
	...
	long lBaseExtra100, int iTotalModifier)
{
	...
	int prodToCommerce = 0;
	{
		...
		{
			const CvInfo* pd = InfoRepo<CvProcessInfo>::get().get((int)eProcess);
			if (pd != NULL)
			{
