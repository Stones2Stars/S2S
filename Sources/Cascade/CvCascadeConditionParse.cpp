...
//	the typed CvCascadeCondition tree, FK-resolving each type/param to its engine id. Mirrors the C# control flow
//	method-for-method (ParseBareString / ParseObject / TryMembership / the string->typed maps).
//

#include "CvGameCoreDLL.h"
#include "CvCascadeConditionParse.h"
...
static CvCascadeCondition* cp_presence(const std::string& type, CvCascScope scope, int min, int max,
                                       CvCascConnection conn, CvCascVicinity vic)
{
	CvCascadeCondition* c = new CvCascadeCondition();
...
static CvCascadeCondition* cp_predicate(CvCascPredKind k, const std::string& param, int min, int max)
{
	CvCascadeCondition* c = new CvCascadeCondition();
...
static CvCascadeCondition* cp_group() { CvCascadeCondition* c = new CvCascadeCondition(); c->kind = CASC_COND_GROUP; return c; }
...
static CvCascadeCondition* cp_parseBareString(const std::string& s);

static CvCascadeCondition* cp_parseObject(const picojson::object& o)
{
	// 1) combinator node
	if (po_has(o, "all") || po_has(o, "any") || po_has(o, "noneOf") || po_has(o, "enabled") || po_has(o, "disabled"))
	{
		CvCascadeCondition* g = cp_group();
		const char* lists[3] = { "all", "any", "noneOf" };
		std::vector<CvCascadeCondition*>* dst[3] = { &g->all, &g->anyOf, &g->noneOf };
		for (int i = 0; i < 3; ++i)
		{
			...
			if (v && v->is<picojson::array>())
			{
				...
				for (size_t j = 0; j < a.size(); ++j) { CvCascadeCondition* c = cascadeParseCondition(a[j]); if (c) dst[i]->push_back(c); }
			}
		}
