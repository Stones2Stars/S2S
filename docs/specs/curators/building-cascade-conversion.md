...
- **#432 trap (found 2026-06-14):** `BonusCommercePercentChanges` is ×100 *without* a `*100` getter (it's
  ...
  heuristic is INCOMPLETE — #432's inventory must also scan ×100-space addends. Canonical scale ruling +
  the closed per-100 set: [DEC-per100-closed-set](../architecture/decisions.md#dec-per100-closed-set) (this trap is the
  named exception the curator must still verify against `Sources/Engine/CvCity.cpp`).
