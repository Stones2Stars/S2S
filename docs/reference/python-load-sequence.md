...
6. **`windowActivation`** → `CvEventManager.onWindowActivation` → **`CvScreensInterface.lateInit()`** on the
   ...
   nothing ([the info plane is write-once-at-load](../architecture/patterns/04-the-info-data-out-contract-what-an.md#-write-once-at-load--a-read-never-creates-and-an-unanswerable-read-fails-loud)). Every screen
