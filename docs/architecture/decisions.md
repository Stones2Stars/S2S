...
The legacy per-100 fields are a closed set of exactly six `…100()` accessors — that set is the curator's entire
de-scale list. **Home:** [fixed-point-and-scales.md §4b](../specs/curators/fixed-point-and-scales.md).
...
### DEC-vicinity
Vicinity = a city's current working-range plots (permissive radius scan, no ownership/worked filter); all special
vicinity variants fold to this one, and a duplicate same-target fold takes `combine:max`. **Home:** [enabler.md §8](../specs/enabler.md).
...
### DEC-parity
Parity is the only goal — exact match, no tolerance band or care-scale; a divergence is a data-collection gap, never
