...
> ⚖ **So does the REVEAL half: a city counts a tile's bonus — every band, on site included — only once its OWN
> team sees it.** Reveal belongs to whoever SEES the plot, never to its owner, so the plot stores which teams see
> its bonus ([the reveal set](13-context-maintained-events.md#maintained-event-driven--never-a-per-turn-recompute))
> and each city books against its own team's bit.
