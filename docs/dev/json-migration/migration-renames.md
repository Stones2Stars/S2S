...
## Predicate vocabulary rewiring (owner 2026-06-22) — the `IS_*`/`HAS_*` split + the `plots` target

> **This section is the SEED of the full per-infotype Rosetta** — [`infotype-translation.md`](infotype-translation.md) extends these old→new (cols 1+3) rows with the legacy-C++ MEANS (col 2) and the consuming cascade machine (col 4). The plot-substrate / predicate slice is mapped there in full; consult it before touching a plot-predicate field on any layer.

The scope×target×predicate model (authoritative: [`docs/modders/datastructure/README.md`](../../modders/datastructure/README.md) §3.2/§3.4/§6) renames the plot-predicate vocabulary and folds `plotTypes`/`seaPlot` into the explicit plural **`plots` target**. **Predicates are target-relative** — evaluated against the deposit's target (`IS_WATER` on `plots` = a water tile, on `units` = a sea unit) → **no `_PLOT`/`_UNIT` suffix.** `IS_*` = what the object IS (type/environment); `HAS_*` = an attribute it HAS. The scope-singular / target-plural rule (a plural target = *all* objects of that kind in scope, filtered by the predicate) is the differentiator.

### Predicate renames (bare predicates in `requires`/`enabled`)
| OLD | NEW | why |
|---|---|---|
| `IS_HILLS` | `HAS_HILLS` | relief is an attribute the plot HAS, not a type it IS |
| `IS_PEAK` | `HAS_PEAK` | (composes onto any environment — `{IS_MARS,HAS_PEAK}` = a Martian peak) |
| `IS_FRESHWATER` | `HAS_FRESHWATER` | adjacency attribute |
| `IS_COASTAL` | `HAS_COAST` | adjacency, "both sides of the coin". ✅ **CONFIRMED (owner 2026-06-22): target-relative.** On `plots` = a plot adjacent to water; in a building `requires` (CITY context) = the city's own `isCoastal` fact — **NOT** a center-plot check ("we should not need a building to know if there's coast on the city plot"). At city scope `HAS_COAST` maps directly to the engine's existing `isCoastal`; the engine/readJson resolves the predicate per the deposit's target |
| `COASTAL_LAND` | `{all:[IS_LAND,HAS_COAST]}` | a coastal land plot = composite; no bespoke predicate |
| KEEP | `IS_WATER`·`IS_FLATLANDS`·`HAS_RIVER`·`HAS_IRRIGATION`·`HAS_FEATURE`·`HAS_TERRAIN`·`HAS_BONUS` | unchanged |
| ADD | `IS_LAND`·`IS_SPACE`·`IS_LUNAR`·`IS_MARS` | environment predicates (target-relative; plot or unit) |

### Structural folds (deposit TARGET — `plotTypes`/`seaPlot` → `plots {predicate}`)
| source field (entity) | OLD curated shape | NEW shape |
|---|---|---|
| `PlotYieldChanges` (Building) | `<yield>.plot.plotTypes.{PLOT_OCEAN\|LAND\|HILLS\|PEAK}.flat` | `<yield>.<scope>.plots.flat {IS_WATER\|IS_LAND\|HAS_HILLS\|HAS_PEAK}` |
| `GlobalSeaPlotYieldChanges` (Building) | `<yield>.empire.flat` (IS_WATER post_process) | `<yield>.empire.plots.flat {IS_WATER}` |
| `RiverPlotYieldChanges` (Building) | `<yield>.plot.river.flat` | `<yield>.<scope>.plots.flat {HAS_RIVER}` (the bespoke `.river` plot-key also folds in). ✅ **owner-confirmed 2026-06-22: FOLD** — `.river` does NOT stay; it folds uniformly with the others into `plots {HAS_RIVER}` (the earlier "may legitimately stay" caveat is resolved) |
| `SeaPlotYieldChanges` (Trait) | `<yield>.empire.seaPlot.flat` | `<yield>.empire.plots.flat {IS_WATER}` |

`plotTypes` and `seaPlot` keys are **RETIRED**. Named-entity keys (`terrains`/`improvements`/`features`) STAY (deliveryguy, modifier-spec §6).

### Curator edit sites (Explore scout scan, 2026-06-22)

**Progress:** ✅ **predicate RENAMES DONE 2026-06-22** — `curate_improvement.py` (6: `IS_PEAK`/`IS_HILLS`/`IS_FRESHWATER`→`HAS_*`, `COASTAL_LAND`→`{IS_LAND,HAS_COAST}`), `curate_building.py` (3: `IS_COASTAL`→`HAS_COAST` ×2, `IS_FRESHWATER`→`HAS_FRESHWATER`), `boolexpr.py` (1: `IS_COASTAL`→`HAS_COAST`).
✅ **structural FOLDS DONE + REGENERATED 2026-06-22** — `plotTypes`/`seaPlot`/`.river` → the `plots` target via `_inject_plots` (`curate_building.py` `PlotYieldChanges`/`GlobalSeaPlotYieldChanges`/`RiverPlotYieldChanges`; `curate_trait.py` `SeaPlotYieldChanges`). Verified: `plotTypes`/`seaPlot`/`.river` = **0** in `Assets/Data`, `plots` present (38 buildings + 4 traits). Trait fold verified via the STORE path against the re-keyed complex variants (`TRAIT_COMPLEX_SEAFARING*` + developing `TRAIT_SEAFARING2/3`) — NOT a raw `--sample` (the complex/vanilla divide).
✅ **`EnabledCivilizationTypes` WIRED 2026-06-22** — civ-whitelist `requires.build.any` `{type:CIVILIZATION_X,scope:empire}` (`curate_building.requires_building` + `curate_unit.requires_unit`); 143 buildings + 41 units carry it.
✅ **doc-sync + ×100 verify DONE 2026-06-22** — `curate_improvement.py` header synced to the `IS_*`/`HAS_*` split; trait SCALARs VERIFIED **human** at the consumption site (`CvPlayer::processTrait` deposits `change<X>(iChange*get<X>())` — ×`iChange` only, never ×100; `CvTraitInfo` has 0 `…100()` getters; the lone `changeExtraCommerce100(100*…*getCommerceChange)` proves its input is human) → **no descale**, docstring corrected. Improvement JSON regenerated (was STALE — old tokens 4/8/9 → 0).
**NEXT — IN SCOPE of THIS rework, NOT "deferred"** (atomic-deliverable ruling, [building-cascade-conversion §0](building-cascade-conversion.md)): the `plots`-target deposits must now be **CONSUMED**. That is **part of what `dry_calc.py` has to do** (owner 2026-06-22) — the offline per-worked-plot yield calc evaluates `plots {predicate}` against each worked plot and deposits per match ([DEC-calc-zero-ride-in](../architecture/decisions.md#dec-calc-zero-ride-in)) — AND the DLL #430 modifier machine ([data-model §4.1](../reference/cascade/data-model.md)). #428 data + #430 engine + `readJson` are ONE deliverable; verification is **per-mechanic parity** (dry vs engine, no averages — [DEC-per-mechanic-parity](../architecture/decisions.md#dec-per-mechanic-parity)), tracked at the `plot yield` row of [parity-mechanic-checklist.md](parity-mechanic-checklist.md).
- **`curate_improvement.py`** `requires_improvement()`: L137 `IS_PEAK`→`HAS_PEAK`, L145 `COASTAL_LAND`→`{IS_LAND,HAS_COAST}`, L160 `IS_HILLS`→`HAS_HILLS`, L162 `IS_FRESHWATER`→`HAS_FRESHWATER`, L166 `IS_PEAK`→`HAS_PEAK`, L174 `IS_FRESHWATER`→`HAS_FRESHWATER`; header comment L20–30.
- **`curate_building.py`**: `requires_building()` L663/665 `IS_COASTAL`→`HAS_COAST`, L669 `IS_FRESHWATER`→`HAS_FRESHWATER`; folds — L141 `GlobalSeaPlotYieldChanges` (`YIELD_FAMILIES`), L142 `RiverPlotYieldChanges`, L275 `PlotYieldChanges` (`TARGET_KEYED` `plotTypes`).
- **`curate_trait.py`** `KEYED` L170 `SeaPlotYieldChanges` (`seaPlot`→`plots {IS_WATER}`).
- **`boolexpr.py`** `_is_pred()` L59 (+21,27) `IS_COASTAL`→`HAS_COAST`.
- Shared bare-predicate constants (KEEP): `curate_improvement.py` L106, `curate_terrain.py` L63, `curate_feature.py` L70 (`HAS_RIVER`/`HAS_IRRIGATION`).

### ⚑ Integer→human (×100) flag (the "surface missing conversions" finding)
`curate_trait.py` SCALAR fields (~L349–352, emitted via `_num(t)`) carry **NO `descale100`** — the docstring says "x100 carried faithfully (#432)" but no descale is imported. **Verify (scale-registry method, at the consumption site) whether any trait SCALAR XML field is ×100-stored** vs human; if any is, add the descale (a per-trait `PER100_TAGS` registry like `curate_building.py` L302/L387). Building/improvement/terrain/feature curators are CLEAN (building descales via `PER100_TAGS`; the plot-leaf curators read human-scale XML).
...
| `YieldsProduced`/`CommercesProduced` | `<family>.city.flat[].{value, enabled:{HAS_CORPORATION:SELF}, per:{anyOf:[prereqBonuses], scope:city}}` | Per-bonus output: `YieldProduced × Σ getNumBonuses(prereqBonus)` → a `per` over the **set** of PrereqBonuses (`anyOf`, new modifier-spec §4 form). Replaces the opaque `produced.perBonus`. **`value` is ×100 in the XML → de-scaled to human** (accessor isn't `…100()`; mapped by math — [fixed-point §4c](../reference/cascade/fixed-point-and-scales.md); fixed 2026-06-21). |
| `iMaintenance` | `maintenance.city.corporation.flat[].{value, enabled, per:{anyOf:…}}` | Per-bonus maintenance, same gating + per-set. **TODO (corp rework):** `value` is likely ×100 too (`calculateCorporationMaintenanceTimes100`) — verify + de-scale; not yet done. |
...
| `iMovement` | `identity.movementCost` | The terrain's base per-plot move cost. ⚠ **SUPERSEDED (owner 2026-06-20): `identity.movementCost`, NOT a `movement.plot.flat` family** — moveCost is a RESOLVER SUBSYSTEM (per-`(unit,edge)`), not a scope-spine modifier family (cascade-migration §moveCost-resolver, modifier.md §6.6). The curator emits `identity.movementCost` via `to_identity` (matching Route/Feature). The earlier 2026-06-16 "movement is a family" framing is retired for the plot-side base cost. |
...
| `MapCategoryTypes` | `identity.mapCategories` **+ `requires.build` gate** | Live placement-gate classification array (`CvPlot::getMapCategories` → `isMapCategory<>` set-membership); matched by attribute, not summed → identity. Renamed for clarity. **ALSO a build gate (owner 2026-06-18):** emitted into `requires.build` as `any[{type:MAPCATEGORY_X, scope:plot}]` → `PRED_HAS_MAP_CATEGORY` (CENTER-plot, uncategorized-plot-valid). Without it space scenarios break (earth buildings offered on space maps). NOT a permanent drop — returns as a viewport/zone gate at the multimap-zone-rework. (The 472-row "DROP" is the separate 0/304 Build/Improvement entity, not buildings.) |
...
| `iMovement` | `identity.movementCost` | Extra move cost, additive onto the terrain-seeded cost. ⚠ **SUPERSEDED (owner 2026-06-20): `identity.movementCost`, NOT `movement.plot.flat`** — same resolver-subsystem ruling as Terrain (curator emits via `to_identity`; modifier.md §6.6). |
...
| `TechYieldChanges` | `<yield>.plot.flat[].{value, enabled:{type:TECH,scope:team}}` ON THE IMPROVEMENT | ⚠ **RESOLVED (owner 2026-06-20, Phase-F): KEEP-ON-SOURCE on the improvement**, NOT inverted onto the tech. The improvement OWNS its tile yield, so its tech-conditioned bump rides on the improvement (plot scope, `enabled` by the team-tech the GIVER) — `curate_improvement.py` post_process. The earlier "inverts onto the tech (`curate_tech` TECH_BOOSTS), DROP here" is retired. x1 human-scale (like base `YieldChanges`). |
...
| `EnabledCivilizationTypes` | `requires.build.any:[[{type:CIVILIZATION_X, scope:empire}]]` (civ-whitelist OR-group) | ✅ **owner ruling 2026-06-22: a civ-whitelist `requires.build` gate** — only the listed civilization(s) may build it (`CvCity::canConstruct`, `getCivilizationType()==getEnabledCivilizationType(i).eCivilization`, CvCity.cpp:2560). The civ-unique building mechanism (242 regular buildings + module). **Was UN-MIGRATED**: the curator coverage-listed it in `REQUIRES_TAGS` but `requires_building()` never read it → emitted NOTHING. Now wired as a `requires.build.any` OR-group of `{type:CIVILIZATION_X, scope:empire}` atoms (empty list = unrestricted). Engine evaluates the `CIVILIZATION_*` atom as "the player's civ is this one" (#430). |
...
| `iCoastalTradeRoutes`/Area*/Global*/National* etc. (~70 scalars) | `<family>.<scope>[.<member>].<unit>` | Corrected scopes. NEW families: **`cityCapture`** (National/Local Capture Probability/Resistance — capturing CITIES, distinct from the §5 unit `capture` gradient), `espionage`{insidiousness,investigation}, `espionageDefense`, `healing`, `foodKept`, `hurryCost`/`hurryAnger`, `cityCapture`, `populationGrowthRate`, `occupationTime` (⚠ **`iPillageGoldModifier` is NOT revived — the earlier "REVIVED §8-ii" is retired**: re-verified DEAD 2026-06-20, no city/player aggregation path, so the curator carries it in `DROP_DEAD`; the live pillage-gold is the promotion-side `PillageChange`). `defense` is grouped (`amount`/`min` floor/`bombardDefense`/`nukeDefense`/`airDefense`/`noEntryLevel`/`dynamicDefense`/`riverDefensePenalty`/recovery/`damageAttackerChance`/`damageToAttacker`/`adjacentDamage`/`repel`). |
...
| PROPERTY-EFFECT BAND REALIGNMENT (`ReplacementBuildings` on pseudobuildings + per-band values) | `apply_property_bands()` post-pass | **Owner 2026-06-19 — pull EDUCATION in line with every other property's bands.** A *pseudobuilding* = any `BUILDING_*` under a `<PropertyBuildings>` in `CIV4PropertyInfos.xml` (placed/removed by the property-band system). **Pseudobuildings NEVER `replace`** (replace = REMOVE): the band post-pass drops every pseudo→pseudo `ReplacementBuildings`→`replaces` edge (the bands STACK cumulatively), and converts a pseudo→**REAL** one to a reversible **`disables`** (the effect turns the target DORMANT, not destroyed — `BUILDING_POLLUTION_BLACKENED_SKIES.disables.buildings:[…telescopes…]`; enabler-spec §5 reversible effect-disable). Education's 4 succession ladders (positive/negative era, argumentative-awareness, blissful-ignorance; 48 successor bands) had their FULL per-band values re-authored as **incremental deltas** (`full[rank]−full[rank−1]`, zero-deltas pruned) so cumulative stacking reproduces the top band's total ("nerf each band for UX"). Crime/disease/tourism/water-pollution were already cumulative (0 replaces) → untouched. Validation: `cascade_sim` commerce mean \|gap\| 29%→14.3%, production 6/6. Full record: `modifier-cascade-known-discrepancies.md` §A.1. |
...
| `EnabledCivilizationTypes` | `requires.build.any:[[{type:CIVILIZATION_X, scope:empire}]]` (civ-whitelist OR-group) | ✅ **owner ruling 2026-06-22: a civ-whitelist `requires.build` gate** (same as Building) — only the listed civ(s) may train it (`CvCity::canTrain`, `getCivilizationType()==getEnabledCivilizationType(i).eCivilization`, CvCity.cpp:2218); the civ-unique UNIT mechanism (Neanderthal units + module). **Was UN-MIGRATED** (curator coverage-listed in `REQUIRES_TAGS`, `requires_unit()` never read it → emitted NOTHING; the mapping mis-filed it under identity). Now a `requires.build.any` OR-group of `{type:CIVILIZATION_X, scope:empire}` atoms. |
