...
All hooks follow the existing patterns in `ai-logging-reference.md` (§5: `log<Domain>AI` +
`streamLogTee`; gated by `gUnitLogLevel`).  No new global is needed — the existing
`Autolog__LogLevelPlayerBBAI` → `gUnitLogLevel` knob already gates `[UNT/*]`.
