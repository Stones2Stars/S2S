...
All hooks follow established patterns (gated by `gPlayerLogLevel` / `gTeamLogLevel`,
`streamLogTee` for `/events` tee, snapshot fields added to the existing `PlayerSnap` /
`CitySnap` structs in `CvHttpServer.cpp`).
