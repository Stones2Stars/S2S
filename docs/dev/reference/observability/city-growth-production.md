...
All proposed hooks follow the existing pattern: gated by `gCityLogLevel` (or a new domain gate if
food gets its own file), carrying `key=value` fields, and streaming via `streamLogTee` into `/events`
as `log` frames.
