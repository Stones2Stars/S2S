...
- **Stage 1 — the combinator:** `all`/`any` × positive/negative. Pure boolean structure + polarity (must-have, or DORMANT-if-present via `noneOf`), fixed by clause shape before any state is read. `all` = AND of its atoms. **`any` is a LIST OF OR-GROUPS that are AND-ed together — "AND-of-ORs", NOT "OR-of-ANDs"** (the engine: `CvCascadeCondition.cpp` `cascadeEvalCondition` — *"each OR-group must contain at least one true leaf"*, and a failing group fails the whole condition). So **`any:[[A,B,C,D]]`** (one group) = *"any one of A,B,C,D"* (a single OR), while **`any:[[A],[B]]`** (two groups) = *A AND B*. This is the shape the curators emit (one OR-group per source-prereq, e.g. `PrereqOrCivics`); `migration-renames.md §Tech` ("list of OR-groups, each AND-ed with the rest") states the same.
