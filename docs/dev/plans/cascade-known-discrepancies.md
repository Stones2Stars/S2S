...
| **`requiresBuild` (1, UNDER)** | `LEECH_CATCHER` — `requires.build = TECH AND (healer-building OR TERRAIN_MARSH)`. The terrain IS evaluated (the parser converts `{type:TERRAIN_MARSH}` → `PRED_HAS_TERRAIN`, CvCascadeReadJson.cpp:180-187 — NOT dropped, an earlier wrong guess), but `PRED_HAS_TERRAIN` checks only the **city CENTER plot** (`pl->getTerrainType()`). Hypothesis: legacy's terrain prereq accepts marsh anywhere in the **workable radius**, so a marsh-adjacent-but-not-on-marsh city passes legacy, fails cascade. | ❓ verify legacy's `PrereqOrTerrain`/`isValidTerrainForBuildings` semantics (center-plot vs radius); align the predicate's scope accordingly |
