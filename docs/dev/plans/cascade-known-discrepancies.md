...
| A2 | **OVER (12): operate-civic + national wonders.** `bRequiresActiveCivics` (the civic gate, in `requires.operate`) and/or `allowed:{empire:1}` national wonders (`GARDEN_CITY`/`KINGS_TOURNAMENT`/`SHRINE`/the huts/…). Cause not yet pinned | ❓ CAUSE TBD | dig — civic-check vs obsolete-prereq-building waiver (cascade waives an obsolete prereq, CvCascadeCondition.cpp:70) vs the cap |
