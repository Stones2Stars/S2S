...
| **`location` (1) — was IMPROVEMENT, not map-category** | `FARM_SUPPLY` — `isValidBuildingLocation` → `isValidTerrainForBuildings` (CvCity.cpp:20474): a **`PrereqOrImprovement`** (`FARM`/`VERTICAL_FARM` on an owned radius plot). The cascade **dropped** `IMPROVEMENT_*` atoms (only TERRAIN/FEATURE/MAPCATEGORY converted) → didn't require the farm → over-offered. Map-category was a RED HERRING (on an earth plot legacy's `isMapCategory` passes too). | ✅ **FIXED** — `PRED_HAS_IMPROVEMENT` (VICINITY, the same `ccVicinityMatch`; owner: "obvious need"). Cleared. **Map-category (separate)**: now IMPLEMENTED as `PRED_HAS_MAP_CATEGORY` (CENTER-plot, uncategorized-plot-valid) — NOT for `FARM_SUPPLY` but because **space scenarios break without it** (owner 2026-06-18: the migration-renames "drop" line was premature — trust-but-verify the doc; corrected there). Sweep no-op on the earth map; returns as a viewport/zone gate at the multimap-zone-rework. |
...
| B3 | **Property-band placement** (`checkPropertyBuildings`, crime/disease/pollution) — per-turn add/remove on a value band ↔ cascade end-state = `requires.operate` property-in-band dormancy | ⚠ SHADOWED 2026-06-18 (`placementSweep`/`[PLACEMENT]`, kind=2); cascade rep PENDING | property gate **replaced at the hard switch** (PropertyEffect, enabler-spec §3). The shadow + the new `PROPERTY_X` band atom + `autoBuild` parsing are in; property-effect buildings still show `reason=noMarker` until their JSON gets `autoBuild` + the `requires.operate` band (serial curation the shadow now drives) |
| B4 | **autoBuild placement** (per-turn autobuild loop) ↔ cascade end-state = `enables` + `autoBuild` placement marker | ⚠ SHADOWED 2026-06-18 (`placementSweep`/`[PLACEMENT]`, kind=1) | wrangle at switch (data-model §4.2b). Shadow built; `identity.autoBuild` now parsed into the cascade. Diagnose remaining divergences → CHANGE/ALIGN, then delete the loop |

**B4 first-run divergence clusters (2026-06-18, live; same character on human player 0 AND AI player 1 — so REAL logic divergences, not UI artifacts).** kind=1 (bAutoBuild loop) divergence cause-tags, by frequency:
- **`replaced` (dominant, ~122–135 in the 250-cap sample)** — cascade=false/legacy=PRESENT: the legacy autobuild loop **keeps a superseded predecessor** in place once a successor is active (it only removes on `PrereqNumOfBuildings`, CvCity.cpp:1473-1485), while the cascade's placement model drops it via `cascadeIsReplacedInCity`. **Disposition TBD** — CHANGE (cascade correctly retires the stale autobuild) vs ALIGN (match legacy's keep-the-predecessor). The biggest single B-i triage item.
- **`requiresOperate` (~20–28)** — `BUILDING_CIVIC_*` (adopted-cultures autobuilds): legacy keeps them after the enabling civic changes; the cascade's `requires.operate` civic clause goes dormant. A dormancy divergence (overlaps B-ii).
- **`place` (~2–7)** — cascade WOULD auto-place (autoBuild + requires hold, not replaced) but legacy absent (e.g. `BUILDING_RESOURCES_GEMSTONE`/`_SAND` on AI city): cascade over-reach vs legacy, or legacy not-yet-placed. Small count; diagnose.
- **`requiresBuild` (~4–5)** / **`allowedCap` (1)** — minor.
- **kind=2 `noMarker` (~87–88)** = the B3 property-band set (un-migrated; expected, drives curation).

**Tooling gap — RESOLVED 2026-06-19:** the **uncapped per-(kind,reason) count histogram is now emitted server-side** (`reasonHistogram` in `placementSweep`/`dormancySweep`; see http-server.md), so triage reads the full distribution, not a sample. The per-cell divergence LIST stays capped at 250 by design (use `?type=full` to dump every cell).
| B5 | **Resource dormancy** (`PrereqBonuses` via `isActiveBuilding`) — losing a continuous resource ↔ cascade `requires.operate` | ⚠ SHADOWED 2026-06-18 (`dormancySweep`/`[DORMANCY]`, `legacyReason=disabled`) | many bonus prereqs currently sit in `requires.build` (grey-only) but should `operate` (Phase-F build-vs-operate); the shadow surfaces these as cascade-active/legacy-disabled, driving the curation |

**B-ii dormancy shadow (2026-06-18):** `GET /diagnostic/dormancySweep` + per-turn `[DORMANCY]` — per BUILT building per city, cascade `requires.operate` (`cascadeOperational`) vs legacy `hasFullyActiveBuilding`. Covers all three legacy mechanisms in one comparison (`legacyReason`: `disabled` = `isDisabledBuilding`, resource/replacement; `religiousLimit` = `isReligiouslyLimitedBuilding`; `active`). B1 (religious) was already MATCH-verified; B5 (resource) + replacement-suppression now have a live shadow. Expected initial divergence: cascade-active / legacy-`disabled` where bonus prereqs still live in `requires.build` (the B5 build→operate curation driver).
