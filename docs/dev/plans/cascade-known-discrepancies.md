...
| A2 | **OVER (12): MIXED cause — narrowed, partly verified.** RULED OUT: civic (cascade `requires.operate` exactly matches `hasValidCivics`, CvPlayer.cpp:26218), map-category (`MAPCATEGORY_EARTH` is on 84% of buildings — not distinguishing on an Earth map), `MaxStartEra` (none). **Sub-cause (a) — in-city building-prereq WAIVER mismatch (verified static):** legacy `CvCity.cpp:2853-2861` requires the prereq `isActiveBuilding` (present + NOT disabled), waiving ONLY on `isObsoleteBuilding`; the cascade atom (CvCascadeCondition.cpp:58-71) *also* waives on the SPECIAL-BUILDING waiver (`isSpecialBuildingNotRequired`) and tests `hasBuilding` not `isActiveBuilding` → over-permissive for the hut/operate-civic buildings carrying in-city building prereqs. **Sub-cause (b) — national wonders** (`GARDEN_CITY`/`KINGS_TOURNAMENT`, `allowed:{empire:1}`, no prereq buildings) — SEPARATE, not isolated (game was down); cap edge or another gate. | ⚠ PARTLY DIAGNOSED | (a) ALIGN — scope the cascade's special-building waiver to UNIT prereqs only + use `isActiveBuilding`; (b) confirm live |
