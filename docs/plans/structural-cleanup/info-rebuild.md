...
| 23 | NEW (wave finding): `HAS_LANDMARK` stays un-folded in `plotAttrs` — `setLandmarkType` emits nothing and the personalized-map pass runs after barbarian-city founding, so 4 shipped option-gated civic `plots` deposits read 0 pending a landmark change event (or a post-generation fold ordering guarantee) | contexts + spine |
...
| 32 | ⛔ NEW (emit rewire, FORWARD HAZARD): **legacy blocks the emits were meant to supersede are live again.** The revert restored engine code the pre-revert tree had DELETED in favour of the spine/grants: the tech first-discoverer block (free unit / prophet / free techs) beside `emitTechAcquired`, `foundReligion`'s free-unit block beside `emitReligionFounded`, and `initFreeState`'s starting gold beside `emitPlayerInit`. Harmless TODAY (the grants apply-loop is unbuilt), but each becomes a **DOUBLE-APPLY the moment that loop lands** — and would then read as a grants bug. The grants apply-loop work item MUST delete them in the same change ([grants-machine.md](grants-machine.md)). | grants apply-loop |
