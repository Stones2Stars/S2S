...
| 20 | **VERIFIED + RE-AIMED (the original finding named the wrong member).** `setMilitaryWorth` is DEAD (zero callers) and simply goes. The LIVE violation is `setPowerValue`: `CvGame.cpp:308-317` divides every great-general unit's power by 10 on the SHARED `CvUnitInfo` when `GAMEOPTION_UNIT_GREAT_COMMANDERS` is off. ⚠ **Not only a contract breach — a live BUG:** an info is loaded once and shared, so the division is cumulative across re-inits (start a second game in one session and it is ÷100). §9's rule applies: the option gate lives at the CONSUMING system and the info serves ungated data. **Fix shape:** delete the mutation AND both setters (so the mutation is unsayable, the contract-not-prohibition principle), add ONE gated calc on the calc surface — `InfoValuation::unitPowerValue(const CvUnitInfo&, bool bGreatCommandersEnabled)`, pure, inputs-in/value-out per the static-function law, following the `InfoValuation::hammerCostPercent` precedent for an option-gated config calc — and route the five type-level readers through it: `CvUnit.cpp:402`, `CvUnit::getPowerValueTotal` (`:27632`/`:27638`, already the single home for the SIZE_MATTERS gate), `CvPlot.cpp:4013`, `CvUnitSort.cpp:80`, `CvTeamAI.cpp:2157`. ⚠ **Sequencing note:** that last site is an AI read, and the AI is ruled LAST — but this is preserving the AI's existing semantics while removing a shared-state mutation, not wiring the AI onto the new getter surface, so it is in scope for the fix rather than for the stage-4 wave. Routing all five keeps behaviour identical; leaving any unrouted silently changes great-general power valuation | `CvGame` + `CvUnitInfo` + the calc surface |
