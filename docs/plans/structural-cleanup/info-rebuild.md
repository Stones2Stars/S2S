...
| 22 | **LANDED** (the arriving type is now routed; the DEPARTING one remains unaddressable because the fact carries only `iType` = the new substrate). Original finding: no consumer case consulted `dependencyForType` for a substrate TYPE — an above-plot deposit conditioned or per-scaled on a named improvement/terrain/feature is never re-marked when that substrate is built/removed; needs its own verification pass against the shipped conditions | `CvModifierConsumer` |
...
| 29 | **VERIFIED — the ABI claim HOLDS for all 8** (`isPermanent`, `getArtInfo`, `getDefaultPlayers`, `isPlayable`/`isAIPlayable`/`getDefaultPlayerColor`, `isLeaders`, `getFlagTexture`). Method + the general answer: [engine.md § Is a symbol really EXE-bound?](../../reference/engine.md). ⚠ The original framing was wrong in a way worth keeping: the EXE has **NO static import from the game-core DLL at all** — it loads it dynamically — so an import-table check answers nothing. The decisive test is that the DLL's exported mangled name appears as a STRING in the EXE (its runtime lookup key). All 8 do. **The same sweep answers the whole surface: 1,205 of the DLL's 1,302 exports are named in the EXE; the other 97 carry NO ABI obligation** and may collapse into the new surface — concentrated in `CvMapExternal` (23), `CvArtFileMgr` (8), `CvPlot` (8), `CvSelectionGroup` (5), `CvGame` (5), `CvPlayer` (4), `CvCity`/`CvUnit` (3 each), plus 32 free functions. | rebuilt infos |
