...
- [`enabler.md`](../../specs/enabler.md) — the cascade enabler's "can I?" model (HAS / CAN GET / HAS THE MEANS TO, `enables` vs `requires`); this legacy `canConstruct`/`canTrain` + reverse-index machinery is what that enabler maps onto and is replacing.
