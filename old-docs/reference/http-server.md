...
- **`GET /diagnostic/modifierSweep?player=N`** — the **#430 MODIFIER pilot shadow** (modifier-cascade-shadow-spec §3.2): for
  each of the player's cities × the PILOT yield channels (food/production/commerce), compares the cascade effective
  (`cascadeModifierApply` over the city-scope deposits) vs legacy `getYieldRate100` (both x1 realized), decomposed
  (`flat`/`percent`/`mult100`) + cause-tagged (`cascadeModifierClassify`) + care-graded (Fine→Meltdown). Returns
  `{parityMode, calcFlow, channel, cities, cells, agree, diverge, careHistogram{Fine..Meltdown}, causeHistogram{"cause:Care"},
  divergences[cap 250]{city, channel, base, flat, percent, mult100, cascade, legacy, delta, cause, care, careName}}`;
  `?type=full` adds the per-cell `all[]` (cap 4000, incl. `legacy100`); `?type=food|production|commerce` (or `?channel=…`)
  scopes to one family. Like the other sweeps the histograms are UNCAPPED + engine-computed. **Expected today: dominated by
  `missingDeposit`/`Bug`** — only BUILDING deposits are wired so far, so any city drawing yield from bonus/civic/tech/
  specialist sources undershoots (the expected parity work, §3.1a), NOT a real alarm.
- **`GET /diagnostic/modifier?player=N&city=M`** — the per-CITY on-demand spot-check (no `type`): the cascade effective +
  flat/percent/mult decomposition + legacy `getYieldRate100` for the three pilot families of one city (the magnitude analogue
  of the `canConstruct`-style gate endpoints; no per-turn-tee timing).
...
- The doTurn harness `cascadeModifierShadow` is the per-turn twin of `modifierSweep`: for **every alive player (human AND
  AI** — an AI sweep is a purer cascade-vs-engine compare) × their cities × the pilot yield channels, it diffs the cascade
  effective vs legacy `getYieldRate100`, emitting `[MODSHADOW]` lines — per-player headline (per-channel divergence counts +
  worst care this turn) at `≥1`, `[MODSHADOW] DIVERGE … cause=… care=…` per-divergence at `≥2`.
