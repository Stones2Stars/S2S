...
> **STATUS: PILOT BUILT (increments 1–3, city-yields) — Assert-clean.** The city-yields pilot of this plan is implemented:
> the parse (`cascadeReadJsonModifiers`), the deposit-flow (`cascadeModifierCitySlot`/`cascadeModifierEffective`, parity
> mode, the swappable `cascadeModifierApply`/`ModifierCalcFlow`), AND the increment-3 shadow surface — `GET /diagnostic/
> modifierSweep` (all-cities triage + uncapped cause/care histograms, `?type=full`, `?type=food|production|commerce` /
> `?channel=` scoping), the per-turn `[MODSHADOW]` line (`cascadeModifierShadow` in `CvGame::doTurn`), and the
> `cascadeModifierClassify` cause-tagger → the Fine→Meltdown care scale (`cascadeModifierCareName`). Scope: **city-scope
> BUILDING deposits on food/production/commerce, parity mode (Mode A)**. Remaining = the §2.2 wider channels/scopes/sources
> + the Mode-B capability flip. (Original status line, now superseded: *proposed 2026-06-19, pending owner inspection.*)
> Companion to
...
- **Build increments (compile-clean each):** (1) parse → per-entity modifier-deposit store + `rjParseModifiers` ✅ DONE +
  ...
  flat entries incl. 4 conditional BONUS-vicinity `enabled` atoms, 0 skipped); (2) ✅ DONE — deposit-flow +
  `cascadeModifierEffective(family,scope,ctx)` + `cascadeModifierParityMode` + the swappable `cascadeModifierApply`/
  `ModifierCalcFlow` (legacy-flat-outside); (3) ✅ DONE — shadow + endpoints: the per-turn `[MODSHADOW]` line
  (`cascadeModifierShadow`, all alive players incl. AI, §3.5), the all-cities `GET /diagnostic/modifierSweep` (triage cap-250
  + UNCAPPED cause/care histograms, `?type=full`, `?type=food|production|commerce` / `?channel=` scoping), AND the **per-entity
  `/diagnostic/modifier?player=N&city=M` query** (decomposed-diff vs `getYieldRate100` on demand, no per-turn-tee timing). The
  cause-tagger + Fine→Meltdown care scale live in `CvCascadeModifier` (`cascadeModifierClassify`/`cascadeModifierCareName`) so
  the endpoint and the doTurn line share ONE definition. **Pre-completion the histograms are dominated by `missingDeposit`/Bug
  — only BUILDING deposits are wired (the expected parity work, §3.1a), not a real alarm.** Remaining = §2.2 channels/scopes.
